...
| G1.5c | Tier-1 deps wiring — engine slice (a) + v1 cognitive ship (replaces b/c) | 🟡 Partial | CC | -a ✅ `58b03e88`+`f40e0d07`+`9d7fce86`; v1 S1-S5 ✅ `9c3da43a`→`7d8a0be3`; Gaspar fix `9505c619`; cleanup `2f94ee36` | G1.5b-pre ✅ + G1.5a ✅ | v1 S1-S5 shipped 2026-04-27 (schema+prompt+runner+route+tests). Gaspar fixed (direct @ai-sdk/anthropic, no Gateway). Gateway dead code deleted (ai-sdk-clients.ts + OTA4 smoke test). S6 (manual prompt review — 5+ investor-grade verdicts) is the remaining gate before G1.5c closes. |
