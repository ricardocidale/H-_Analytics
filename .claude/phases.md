...
| G1.5b | Defaults & Assumptions cascade for 5 Funding fields — schema columns + DEFAULT_* constants + model_defaults seed + Admin Steady-State UI + Funding-tab Assumption inputs + form-hook wiring | ⏳ Pending | Replit | — | G1.5b-pre ✅ Shipped | Parent: `g1.5b-funding-cascade.md` → child a (schema + Defaults + Admin UI) + child b (Assumption UI + form hook). Per `inflation-cascade.md` three-tier rule. |
