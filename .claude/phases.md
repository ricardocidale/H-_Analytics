...
| G1.5c | Tier-1 deps wiring (a + b + c) — engine slice + orchestrator wrap + route slice | 🟡 Partial — ⚠️ -b BLOCKED | CC | -a ✅ `58b03e88`+`f40e0d07`+`9d7fce86` | G1.5b-pre ✅ + G1.5a ✅ | -a (engine slice) ✅ shipped 2026-04-27. **-b BLOCKED on architectural decision** — `synthesis-schema.ts CANONICAL_RESEARCH_FIELDS` enum doesn't include funding keys; Option A (adapter shim) requires modifying shared synthesis surface, Option B (per-Specialist purpose-built pipeline) is ~300 LOC + 3 system prompts, Option C (pluggable schemas) is a larger refactor. See `.claude/replit-handoffs/g1.5c-tier1-deps-b.BLOCKED.md`. -c blocked by -b. AnalystButton click is the only trigger. |
