...
| G1.6-v1 | Property Risk Intelligence (Daniela / D) v1 route handler wiring — extends `POST /api/analyst/refresh` with `scope:"property"` + `propertyId` + `property.risk-intelligence` branch | ✅ Shipped | CC | this session | Daniela runner ✅ (UNWIRED removed), G1.5c ✅ | Route handler wired; `modelConstantToCountryInflationOutlook` maps model_constants row to CountryInflationOutlook; honest-null when Isadora hasn't populated. Replit follow-up: update PropertyEdit.tsx to pass scope:"property"+propertyId. |
| G2-v1 | Revenue (B) v1 — single-shot Opus + careful prompt + rich context | ✅ Shipped | CC | `fe6dcbe3` | G1.5c ✅ (1 session soak done 2026-04-28) | 6 files (1342 ins): output-schema, prompt-input-builder, orchestrator-adapter (12 canned comps), prompt, runner, analyst-admin.ts branch. Replit follow-up: wire `<AnalystButton>` on PropertyUnderwritingTab → `specialistId:"mgmt-co.revenue"`. |
