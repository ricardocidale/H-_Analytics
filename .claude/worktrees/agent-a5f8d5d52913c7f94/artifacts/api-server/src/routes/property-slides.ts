 ...
 * Returns the full slide data payload (property, photos, financials, siblings,
 * visionText) as JSON — consumed by the interactive slide viewer artifact.
 * Uses requireAuth (not requireAdmin) so the viewer tab works for any
 * authenticated user who has access to the property.
 */
router.get("/api/properties/:id/slides/view", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Photos — return URLs, not bytes
    const rawPhotos = await storage.getPropertyPhotos(propertyId);
    const sortedPhotos = [...rawPhotos].sort((a, b) => {
      if (a.isHero !== b.isHero) return a.isHero ? -1 : 1;
      return (a.sortOrder ?? 0) - (b.sortOrder ?? 0);
    });
    const photos = sortedPhotos.slice(0, MAX_PHOTOS).map(ph => ({
      id: ph.id,
      url: `/api/property-photos/${ph.id}/image`,
      isHero: ph.isHero,
      sortOrder: ph.sortOrder ?? 0,
      caption: ph.caption ?? "",
    }));

    // Siblings — other properties for the pipeline slide
    let siblings: Array<Record<string, unknown>> = [];
    try {
    ...
    try {
      ...
      const compute = await recomputeSinglePropertyAndStamp({
        property: stamped,
        globalAssumptions: globalAssumptions as GlobalInput,
        projectionYears: PROJ_YEARS_DEFAULT,
      });
      const stampedLoanProps = stamped as unknown as Parameters<typeof calculateLoanParams>[0];
      const unified = aggregateUnifiedByYear(
        compute.monthly,
    ...
    // Vision text — LLM with deterministic fallback + 10 s viewer timeout
    const p = property as Record<string, unknown>;
    const visionInput = {
      id: property.id,
      name: property.name,
    ...
    const visionText = await Promise.race([
      ...
      new Promise<ReturnType<typeof buildPropertyVisionFallback>>(resolve =>
        setTimeout(() => {
          logger.warn(`Vision text LLM timed out for property ${propertyId}, using fallback`, "property-slides");
          resolve(buildPropertyVisionFallback(visionInput));
        }, VISION_TIMEOUT_MS),
      ),
    ]);
