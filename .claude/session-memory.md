...
- **Replit (Phase 5C — 1 commit `6a18d8cf`):** added `DEFAULT_CAPITAL_RAISE_1_DATE` / `DEFAULT_CAPITAL_RAISE_2_DATE` to `shared/constants.ts`; adopted across `shared/schema/config.ts`, `server/syncHelpers.ts`, `server/seeds/properties.ts`, `Section04GlobalAssumptions.tsx` (8 literal substitutions). Verification: TS 0, Lint 0, vocab 11/11, test:summary PASS, Verify UNQUALIFIED. D-1-B added to `audit-inventory.md` as closed. Awaiting Phase 5B handoff.
...
- **Replit (Phase 5B — 2 commits `f2c90e04`, `5dd1a5f4`):** Phase 5B was reframed during handoff drafting — the entire `server/ai/kb/` directory turned out to be orphaned (added in `640e889f`, never wired into RAG pipeline). Ported 4 high-value chunks (Founder Background, International Depreciation, Research Workflow, Governed Model Constants) into `server/ai/kb-content.ts` with vocabulary cleanup, then `rm -rf server/ai/kb/` (19 files, ~900 lines). Zero application-code references remain. Verification after each commit: TS 0, Lint 0, vocab 11/11, test:summary PASS, Verify UNQUALIFIED. **Re-index (Task 5B-3) pending user action** — the `/api/admin/vector-store/reindex/knowledge-base` endpoint requires an admin session that Replit Agent cannot mint from CLI. User must trigger via Admin UI; expected `chunksIndexed` delta ≈ +4. D-2-B closed in `audit-inventory.md`.
