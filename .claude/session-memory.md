...
- Caveat preserved: `fbb7429d` claimed two auto-memory files were written (`llm_vendor_roster_and_prompt_engineer.md`, `specialist_consolidation_permission.md`) — they DO NOT exist anywhere. Persistence failed or the prior session over-claimed. Re-create from doctrine in `llm-vendor-roster.md` + ADR-007 §2 if memory is needed.
