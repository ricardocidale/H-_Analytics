...
Business simulation portal for **Hospitality Business Group**. Models a boutique hospitality management company alongside individual property SPVs with monthly and yearly financial projections. GAAP-compliant (ASC 230, ASC 360, ASC 470). 1056 source files, ~174K lines, 4,536 tests across 187 test files. 15-phase verification pipeline. Hosted on Replit.
...
| Proof System | `.claude/skills/proof-system/SKILL.md` | 4,536 tests across 187 files, 583 golden values, 15-phase verification pipeline |
...
**Commands**: `npm test` (all 4,536 tests, 187 files) · `npm run verify` (15-phase GAAP) · `npm run health` (tsc+tests+verify+doc harmony)
...
- **Testing Dashboard (Task 5.4)** — Admin Verification > Testing tab. `TestingDashboard.tsx` component with stat cards (source files, test files, total tests, scan duration), source code breakdown bar chart, `as any` budget progress bar, and audit findings list with severity badges (critical/warning/info). Server endpoints `GET/POST /api/admin/testing-dashboard/last|run` in `server/routes/admin/tools.ts`. Parses `script/stats.ts` + `script/audit-quick.ts` output. Rate-limited to 1 run/min.
- **Rebecca Screen Context Awareness (Phase 6A)** — `currentPage` field added to `RebeccaContext` in `panel-manager.ts`. `derivePageLabel()` maps URL paths to semantic page labels (property-edit, scenario-comparison, admin, etc.). Both `sendAutoGreeting` and `sendMessage` pass `currentPage` to `/api/chat`. Server injects `Currently viewing: {pageDescription}` into CURRENT USER context block. Chat request schema extended with `currentPage: z.string().max(100).optional()`.
- **Rebecca Proactive Suggestions (Phase 6B)** — Server parses `⚠️ Observations:` from entity summary, returns `observations` array in chat response. Client-side `parseObservationField()` maps observations to field keys (revShareFB, revShareEvents). Observations feed into `useRebeccaInsightStore` as insight banners with "Ask Rebecca about this" action buttons. Deduplication via hash prevents repeat insights for the same entity/field.
- **as any budget** — Server budget raised from 70 → 85 to accommodate CLI-generated code in `rebecca-context-builder.ts` and `server-export-data.ts`.
