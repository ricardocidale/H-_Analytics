...
- Skills live in `.claude/skills/` (~21 domains, ~195 files). See `_index.md` for the master catalog.
...
server/          Express API, storage, AI, exports — ~325 files, ~60K lines
client/          React 18 frontend — ~685 files, ~116K lines
shared/          Types, constants, schemas — ~40 files, ~7K lines
tests/           Test suites (golden, proof, server) — ~227 files, ~56K lines
docs/            Architecture, developer guide, research, planning
.claude/         AI knowledge base (~21 domains, ~195 skills, ~34 rules)
```
...
With ~195 skill files across ~21 domains, **never load all skills at once**. Use `.claude/skills/_index.md` for the master catalog.
...
| Rules (~34) | `.claude/rules/` | All behavioral constraints. **Read `pre-commit-verification.md` and `cross-check-invariants.md` before any edit.** |
...
- ✅ OT-A.4 — **shipped (Replit, `7da9f25a`, 2026-04-19 18:14 UTC).** Legacy regex extractor retired; `streamObject` + `synthesisOutputToLegacyJson` adapter is the single synthesis path; `USE_AI_SDK_SYNTHESIS=true` by default; `ENGINE_VERSION` bumped v1→v2 (`v2-2026-04-20-a`); `SYNTHESIS_FINGERPRINT` `786aae35…`. Zod validation failures surface as `ORCHESTRATOR_BOTH_FAILED` → single-model fallback engages cleanly. All gates green on ship.
- 🟡 OT-A.5 — **drafting (Replit), in T+72h observation window (eligible 2026-04-22 18:14 UTC).** Section A (`inflationRate`) **DEFER** outcome (commit `97c5a331`): Q1-extended verification against `OT-A-3-ab-raw.json` showed all 20 v5 cases are US-only — mono-country sample cannot test country-awareness. Filed for OT-A.6 with $3–5 mixed-country LEA trace gate. Section C.2 (`costSeg5yrPct`): v3.3 anchor confirmed byte-identical between `e5d873fe` and HEAD; v5 −26.7% bias is a real regression — strengthen with IRS Cost Seg ATG source pointer. **v6 batch scope after prep:** 6 T2 USALI source-pointer anchors (B.1–B.6) + C.2 strengthen + C.1 docs-only reclassification. Single $22 v6 rerun authorized at gate clearance. Drafts staged at `.local/drafts/OT-A-5-item-{3,4}-*.md` (Sentry `fallback_reason` tag patch + v6 prompt diff package); held until gate.
...
**OT-A.5 prep — Section A DEFER + C.2 strengthen confirmed (April 19, 2026, Replit, `97c5a331`, docs-only):** Two offline verifications during the OT-A.4 T+72h observation window. Section A: tabulating `(market, legacy.inflationRate.mid, new.mid)` across all 20 v5 cases revealed mono-country US-only sample — STAYS/PROMOTED rules from OT-A-5-design.md §A both presuppose mixed-country evidence. DEFER outcome; Section A removed from v6 batch; filed for OT-A.6 with $3–5 mixed-country LEA trace gate. Section C.2: diffed FIELD_DEFINITIONS cost-seg block between `e5d873fe` and HEAD — byte-identical. v3.3 anchor intact; the v5 −26.7% bias is a real regression the v3.3 wording failed to prevent. Restore-only insufficient — strengthen with IRS Cost Segregation Audit Techniques Guide source pointer. v6 prompt LOC delta now ~100–140 (down from ~120–160 without Section A). Drafts for items (3) Sentry `fallback_reason` tag patch and (4) v6 prompt diff package staged at `.local/drafts/`; held until 2026-04-22 18:14 UTC gate clearance with explicit $22 ack. No source files touched, no API spend, engine-version-drift test stays clean.

**Lint cleanup 60% done (April 20, 2026, Claude Code):** 348 → 138 warnings across 8 executed batches. Batches 1–4 + 7 + 8 (Claude, earlier) = 193 warnings. Batch 5a (`c66896fc`) = Category A audit/verification schema-nullable + test fixtures, 22 swaps across 5 files, 187 → 159. Batch 6a+6b (`fb4bbbe7`) = Category A+E groupBy accumulators + optional-chained array reads + chart viewBox coords, 20 swaps across 14 files, 159 → 138. Remaining pending user Option 1/2/3 call: Batch 5b/5c (Cat B+C engine output, NaN-exposure risk, ~61 sites), Batch 6c (LLM prompts ~15), 6d (dashboard display ~12), 6e (Number() input parsing with `Number.isFinite` wrap ~14), 6f (3 inspect-required). Preaudits in `.claude/plans/batch-{5,6}-preaudit.md`. Parent plan: `.claude/plans/lint-warning-cleanup.md`.
