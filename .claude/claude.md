...
- **Defaults ≠ Assumptions — DO NOT CONFUSE.** **Assumptions = user-facing working variables** (the numbers a user types, saves, and scenarios run on, on the front of the app). **Defaults = admin-only seed values** loaded into the DB to initialize a fresh tenant. The word *"assumption"* in any UI label, button, tooltip, error message, AI agent text, or documentation **always means the user's working variable** — never a default. The word *"default"* must not appear in user-facing copy outside the Admin section. Conflating these has caused real production losses (admin-only routing on user pages, reset buttons wiping user work, seed values treated as authoritative). Full rule in `.claude/skills/vocabulary/SKILL.md` §0.
