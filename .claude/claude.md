...
Business simulation portal for **Hospitality Business Group**. Models a boutique hospitality management company alongside individual property SPVs with monthly and yearly financial projections. GAAP-compliant (ASC 230, ASC 360, ASC 470). 953 source files, ~155K lines, 4,024 tests across 172 test files. Hosted on Replit.
...
| Proof System | `.claude/skills/proof-system/SKILL.md` | 4,024 tests across 172 files, 583 golden values, verification commands |
...
**Commands**: `npm test` (all 4,024 tests, 172 files) · `npm run verify` (8-phase GAAP) · `npm run health` (tsc+tests+verify+doc harmony)
