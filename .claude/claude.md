...
Business simulation portal for **Hospitality Business Group**. Models a boutique hospitality management company alongside individual property SPVs with monthly and yearly financial projections. GAAP-compliant (ASC 230, ASC 360, ASC 470). 1006 source files, ~172K lines, 4,325 tests across 178 test files. 11-phase verification pipeline. Hosted on Replit.
...
| Proof System | `.claude/skills/proof-system/SKILL.md` | 4,325 tests across 178 files, 583 golden values, 11-phase verification pipeline |
...
**Commands**: `npm test` (all 4,325 tests, 178 files) · `npm run verify` (11-phase GAAP) · `npm run health` (tsc+tests+verify+doc harmony)
...
npm run test:summary   # All 4,325 tests, 178 files (~35s)
