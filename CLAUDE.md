...
| [x] | **COMPLETE — `DEFAULT_ADR_GROWTH_RATE`** (session 27, 2026-05-19): constant already absent from codebase — replaced by `SEED_ADR_GROWTH_RATE = 0.03` (Cat 5, `lib/shared/src/constants.ts`) during Phase 2 wiring. Engine reads `adrGrowthRate` from `modelDefaultsInput` (route layer → DB). Sentinel test `defaults-model-defaults-input.test.ts` verified. §2 T1-4 campaign **fully complete** — no remaining `DEFAULT_*` financial constants in source. | T1-4 closure |
...
| 2026-05-19 | **`DEFAULT_PROPERTY_INCOME_TAX_RATE` + `DEFAULT_LAND_VALUE_PERCENT` retired (§2 T1-4, session 24).** Commit A (`7f8c6fd57`) rewired all ~16 consumers across calc, engine, field-registry, portal, seeds. `RESEARCH_TAX_RATE_25_PCT = 0.25` added to `constants-research.ts` for depreciation-basis research surface. `hold-vs-sell.ts` `land_value_pct` tightened to required (no `??`). `PropertyAuditInput` tightened; `TestCase.property` intersection type added. All `?? DEFAULT_*` chains removed. Commit B (`8c133659c`) deleted both definitions with tombstone comments. |
| 2026-05-19 | **Phase 2 shipped — Valentina research specialist + engine wiring (session 27).** `computePropertyDefaults` reads `adrGrowthRate`, `maxOccupancy`, `adrByTier` from `ModelDefaultsInput` (route layer). `ModelDefaultsMissingTierError` replaces `?? 220` fallback. Valentina: pure LLM specialist batches eligible `model_defaults` rows, writes `proposed_*` columns. Admin route `POST /api/admin/model-defaults/research`, Rebecca tool `trigger_model_defaults_research`, migration guard `admin-resources-015`. |
