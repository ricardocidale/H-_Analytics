...
| [ ] | **PAUSED — §2 T1-4 campaign**: `DEFAULT_ADR_GROWTH_RATE` (blocked — must wire `computePropertyDefaults` to read from `model_defaults` before retiring); `DEFAULT_TRAVEL_COST_PER_CLIENT`+`DEFAULT_IT_LICENSE_PER_CLIENT` (blocked — same reason). Do NOT resume until Phase 2 is planned. | T1-4 incremental |
...
| Date | Change |
|---|---|
| 2026-05-18 | **Numeric architecture brainstorm — §2 campaign PAUSED.** `DEFAULT_ADR_GROWTH_RATE` retirement reverted (broke typecheck, regressed ratchet, inline 0.03 in engine/calc). Three-pillar architecture documented in `docs/brainstorms/numeric-architecture-requirements.md`: financial assumptions belong in `model_defaults` (Analyst-researched), not TS constants or inline literals. `computePropertyDefaults` wiring to `model_defaults` deferred to Phase 2. Campaign blocked until Phase 2 planned. |
