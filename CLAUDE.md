...
| [ ] | Tier 1 remaining (bounded, 1-3 hr each): `DEFAULT_ADR_GROWTH_RATE` (needs schema `.default(0.03)` migration first — `notNull()` column with no default today); `DEFAULT_TRAVEL_COST_PER_CLIENT`+`DEFAULT_IT_LICENSE_PER_CLIENT` (paired, audit first — value conflict between `constants.ts` 5000/3600 and `constants-staffing.ts` 3600/3000) | T1-4 incremental |
