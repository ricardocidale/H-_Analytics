...
/**
 * Pure aggregation function: maps raw API data from three sources into a
 * single sorted UnifiedRun array. Active runs are sorted first, then by
 * startedAt desc.
 *
 * Exported so it can be unit-tested independently of React hooks.
 */
export function aggregateUnifiedRuns(
  irisStatus: IrisStatus | undefined,
  slideRuns: SlideFactoryRun[],
  schedulerRuns: SchedulerRunRow[],
): UnifiedRun[] {
...
export function useUnifiedRuns(): { runs: UnifiedRun[]; isLoading: boolean } {
  ...
  const runs = useMemo<UnifiedRun[]>(
    () => aggregateUnifiedRuns(irisStatus, slideRuns, schedulerRuns),
