...
>>>>>>> fdb3de90 (Git commit prior to merge)
}
...
// ---------------------------------------------------------------------------
// get_market_rates + update_market_rate (U7)
// ---------------------------------------------------------------------------

async function toolGetMarketRates(
  args: Record<string, unknown>,
  _ctx: ToolContext,
): Promise<{ result: unknown }> {
...
async function toolUpdateMarketRate(
  args: Record<string, unknown>,
  ctx: ToolContext,
): Promise<{ result: unknown; dataChanged?: DataChangedEntry }> {
  const authError = await requireAdminCtx(ctx);
...
// ---------------------------------------------------------------------------
// get_global_assumptions — read current global assumptions
// ---------------------------------------------------------------------------

async function toolGetGlobalAssumptions(ctx: ToolContext): Promise<{ result: unknown }> {
  const ga = await storage.getGlobalAssumptions(ctx.userId);
  if (!ga) return { result: { error: "Global assumptions not found" } };
  return { result: ga };
}
...
// ---------------------------------------------------------------------------
// list_kb_entries — admin-only KB entry list
// ---------------------------------------------------------------------------

async function toolListKbEntries(args: Record<string, unknown>, ctx: ToolContext): Promise<{ result: unknown }> {
  const authError = await requireAdminCtx(ctx);
  if (authError) return authError;
  const category = typeof args.category === "string" ? args.category : undefined;
  const entries = await storage.getRebeccaKBEntries(category);
...
// ---------------------------------------------------------------------------
// get_kb_entry — retrieve a single KB entry by ID
// ---------------------------------------------------------------------------

async function toolGetKbEntry(args: Record<string, unknown>, _ctx: ToolContext): Promise<{ result: unknown }> {
  const id = typeof args.id === "number" ? args.id : Number(args.id);
  if (!id || isNaN(id)) return { result: { error: "id must be a positive integer" } };
  const entry = await storage.getRebeccaKBEntry(id);
  if (!entry || !entry.isActive) return { result: { error: "Not found" } };
  return { result: { id: entry.id, title: entry.title, content: entry.content, category: entry.category, source: entry.source } };
}

// ---------------------------------------------------------------------------
// share_scenario — share with another user by email
// ---------------------------------------------------------------------------

async function toolShareScenario(args: Record<string, unknown>, ctx: ToolContext): Promise<{ result: unknown; dataChanged?: DataChangedEntry }> {
  const scenarioId = typeof args.scenarioId === "number" ? args.scenarioId : Number(args.scenarioId);
  const recipientEmail = typeof args.recipientEmail === "string" ? args.recipientEmail.trim() : "";
  if (!scenarioId || isNaN(scenarioId)) return { result: { error: "scenarioId must be a positive integer" } };
  if (!recipientEmail) return { result: { error: "recipientEmail is required" } };
  ...
  const sharer = await storage.getUserById(ctx.userId);
  if (!sharer) return { result: { error: "Authenticated user not found" } };
  if (recipientEmail === sharer.email) return { result: { error: "You cannot share scenarios with yourself" } };

  const scenario = await storage.getScenario(scenarioId);
  if (!scenario || scenario.userId !== ctx.userId) return { result: { error: "Not found" } };

  const recipient = await storage.getUserByEmail(recipientEmail);
  if (!recipient) return { result: { shares: [], recipientName: null } };
...
// ---------------------------------------------------------------------------
// delete_property_photo — remove a photo from property gallery
// ---------------------------------------------------------------------------

async function toolDeletePropertyPhoto(args: Record<string, unknown>, ctx: ToolContext): Promise<{ result: unknown; dataChanged?: DataChangedEntry }> {
  const propertyId = typeof args.propertyId === "number" ? args.propertyId : Number(args.propertyId);
  const photoId = typeof args.photoId === "number" ? args.photoId : Number(args.photoId);
  if (!propertyId || isNaN(propertyId)) return { result: { error: "propertyId must be a positive integer" } };
  if (!photoId || isNaN(photoId)) return { result: { error: "photoId must be a positive integer" } };
  ...
  const photo = await storage.getPhotoById(photoId);
  if (!photo || photo.propertyId !== propertyId) return { result: { error: "Not found" } };
  ...
  await storage.deletePropertyPhoto(photoId);
  return { result: { success: true }, dataChanged: { entityType: "property", entityId: propertyId } };
}

// ---------------------------------------------------------------------------
// set_hero_photo — set the hero (primary) image for a property
// ---------------------------------------------------------------------------

async function toolSetHeroPhoto(args: Record<string, unknown>, ctx: ToolContext): Promise<{ result: unknown; dataChanged?: DataChangedEntry }> {
  const propertyId = typeof args.propertyId === "number" ? args.propertyId : Number(args.propertyId);
  const photoId = typeof args.photoId === "number" ? args.photoId : Number(args.photoId);
  if (!propertyId || isNaN(propertyId)) return { result: { error: "propertyId must be a positive integer" } };
  if (!photoId || isNaN(photoId)) return { result: { error: "photoId must be a positive integer" } };
...
// ---------------------------------------------------------------------------
// update_company — admin-only company field patch
// ---------------------------------------------------------------------------

async function toolUpdateCompany(args: Record<string, unknown>, ctx: ToolContext): Promise<{ result: unknown; dataChanged?: DataChangedEntry }> {
  const authError = await requireAdminCtx(ctx);
  if (authError) return authError;
  ...
  if (Object.keys(patch).length === 0) return { result: { message: "No changes applied" } };

  const [updated] = await db.update(companies).set(patch).where(eq(companies.id, id)).returning({ id: companies.id, name: companies.name });
  return { result: { success: true, id: updated.id, name: updated.name }, dataChanged: { entityType: "company" as const, entityId: id } };
}