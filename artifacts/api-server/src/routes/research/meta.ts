...
export function registerResearchMetaRoutes(app: Express) {
  app.get(
    "/api/admin/intelligence/freshness-counts",
    requireAdmin,
    async (_req, res) => {
      try {
        ...
        const latestRuns =
          await storage.getLatestCompletedRunsPerEntity("property");

        const runMap = new Map<
          number,
          { completedAt: Date; durationMs: number | null }
        >();
        for (const r of latestRuns) {
          runMap.set(Number(r.entityId), {
            completedAt: new Date(r.completedAt),
            durationMs: r.durationMs ? Number(r.durationMs) : null,
          });
        }

        const runningEntityIds =
          await storage.getRunningResearchEntityIds("property");
        ...
        for (const p of allProperties) {
          ...
          const daysAgo = Math.floor(
            (Date.now() - completedTs) / (1000 * 60 * 60 * 24),
          );
          const assumptionTs = p.lastAssumptionChangeAt
            ? new Date(p.lastAssumptionChangeAt).getTime()
            : 0;
  ...
  app.get("/api/research/avg-duration", requireAuth, async (req, res) => {
    try {
      ...
      const latestRuns =
        await storage.getLatestCompletedRunsPerEntity(entityType);
      let totalDurationMs = 0;
      let durationCount = 0;
      for (const r of latestRuns) {
        if (r.durationMs) {
  ...
  app.get("/api/research/last-full-refresh", requireAuth, async (req, res) => {
    try {
      const lastRefresh = await storage.getLastFullResearchRefresh(
        getAuthUser(req).id,
      );
