 ...
 *
 * Two-pass verification of the Builder's Slide5Payload:
 *   Pass 1 — deterministic: slide5PayloadSchema.safeParse (Zod)
...
export interface ElisaInspectorVerdict {
...
const INSPECTOR_PASS2_TOOL: Anthropic.Tool = {
  ...
  description:
    "Report whether the assembled Slide 5 editorial payload is acceptable " +
...
/**
 * Run Elisa-03: verify the Builder's payload via Zod (Pass 1) and Opus 4.7
 * vision (Pass 2). Returns a verdict with status and optional notes.
 */
export async function runElisaInspector(
  payload: Slide5Payload,
  canonicalPngKey: string,
): Promise<ElisaInspectorVerdict> {
  // ── Pass 1 — deterministic Zod validation ──────────────────────────────────
  const parsed = slide5PayloadSchema.safeParse(payload);
  if (!parsed.success) {
    const issues = parsed.error.issues
      .map((i) => `${i.path.join(".")}: ${i.message}`)
      .join("; ");
    logger.warn(`[elisa-03] Pass 1 BLOCK — schema issues: ${issues}`, "slide-factory");
    return { status: "block", notes: `Schema validation failed: ${issues}` };
  }

  logger.info("[elisa-03] Pass 1 PASS — schema valid", "slide-factory");
  ...
  try {
    ...
    logger.warn(
      `[elisa-03] Pass 2 skipped — canonical PNG unavailable: ${String(err)}`,
      "slide-factory",
    );
  ...
  const payloadSummary =
    `transformationDescription: ${payload.transformationDescription ? `"${payload.transformationDescription.text}"` : "(none)"}\n` +
    `transformationRows (${payload.transformationRows?.length ?? 0}):\n` +
    (payload.transformationRows
      ?.map(
        (r, i) =>
          `  ${i + 1}. Feature: "${r.feature.text}" | Existing: "${r.existing.text}" | Proposed: "${r.proposed.text}"`,
      )
      .join("\n") ?? "  (none)");
  ...
  const response = await anthropic.messages.create({
    ...
    system:
      "You are Elisa-03, the Slide 5 Inspector for the H+ Analytics investor deck factory. " +
  ...
  if (!toolBlock) {
    logger.warn("[elisa-03] Pass 2 — no tool call; defaulting to approved", "slide-factory");
    return { status: "ok", notes: null };
  }
  ...
  logger.info(
    `[elisa-03] Pass 2 ${verdict.approved ? "APPROVED" : "BLOCKED"} — ${verdict.notes ?? "no notes"}`,
    "slide-factory",
  );
