 ...
 *
 * Two-pass verification of the Builder's Slide1Payload:
 *   Pass 1 — deterministic: slide1PayloadSchema.safeParse (Zod)
...
export interface SofiaInspectorVerdict {
...
const INSPECTOR_PASS2_TOOL: Anthropic.Tool = {
  ...
  description:
    "Report whether the assembled Slide 1 editorial payload is acceptable " +
...
/**
 * Run Sofia-03: verify the Builder's payload via Zod (Pass 1) and Opus 4.7
 * vision (Pass 2). Returns a verdict with status and optional notes.
 */
export async function runSofiaInspector(
  payload: Slide1Payload,
  canonicalPngKey: string,
): Promise<SofiaInspectorVerdict> {
  // ── Pass 1 — deterministic Zod validation ──────────────────────────────────
  const parsed = slide1PayloadSchema.safeParse(payload);
  if (!parsed.success) {
    const issues = parsed.error.issues
      .map((i) => `${i.path.join(".")}: ${i.message}`)
      .join("; ");
    logger.warn(`[sofia-03] Pass 1 BLOCK — schema issues: ${issues}`, "slide-factory");
    return { status: "block", notes: `Schema validation failed: ${issues}` };
  }

  logger.info("[sofia-03] Pass 1 PASS — schema valid", "slide-factory");
  ...
  try {
    ...
    logger.warn(
      `[sofia-03] Pass 2 skipped — canonical PNG unavailable: ${String(err)}`,
      "slide-factory",
    );
  ...
  const payloadSummary =
    `headerSubtitle: ${payload.headerSubtitle ? `"${payload.headerSubtitle.text}"` : "(none)"}\n` +
  ...
  if (!toolBlock) {
    logger.warn("[sofia-03] Pass 2 — no tool call; defaulting to approved", "slide-factory");
    return { status: "ok", notes: null };
  }
  ...
  logger.info(
    `[sofia-03] Pass 2 ${verdict.approved ? "APPROVED" : "BLOCKED"} — ${verdict.notes ?? "no notes"}`,
    "slide-factory",
  );
