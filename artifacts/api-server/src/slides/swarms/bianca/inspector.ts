 ...
 *
 * Two-pass verification of the Builder's Slide2Payload:
 *   Pass 1 — deterministic: slide2PayloadSchema.safeParse (Zod)
...
export interface BiancaInspectorVerdict {
...
const INSPECTOR_PASS2_TOOL: Anthropic.Tool = {
  ...
  description:
    "Report whether the assembled Slide 2 editorial payload is acceptable " +
...
/**
 * Run Bianca-03: verify the Builder's payload via Zod (Pass 1) and Opus 4.7
 * vision (Pass 2). Returns a verdict with status and optional notes.
 */
export async function runBiancaInspector(
  payload: Slide2Payload,
  canonicalPngKey: string,
): Promise<BiancaInspectorVerdict> {
  // ── Pass 1 — deterministic Zod validation ──────────────────────────────────
  const parsed = slide2PayloadSchema.safeParse(payload);
  if (!parsed.success) {
    const issues = parsed.error.issues
      .map((i) => `${i.path.join(".")}: ${i.message}`)
      .join("; ");
    logger.warn(`[bianca-03] Pass 1 BLOCK — schema issues: ${issues}`, "slide-factory");
    return { status: "block", notes: `Schema validation failed: ${issues}` };
  }

  logger.info("[bianca-03] Pass 1 PASS — schema valid", "slide-factory");
  ...
  try {
    ...
    logger.warn(
      `[bianca-03] Pass 2 skipped — canonical PNG unavailable: ${String(err)}`,
      "slide-factory",
    );
  ...
  const payloadSummary =
    `operationalModelText: ${payload.operationalModelText ? `"${payload.operationalModelText.text}"` : "(none)"}\n` +
    `revenueBullet: ${payload.revenueBullet ? `"${payload.revenueBullet.text}"` : "(none)"}\n` +
  ...
  const response = await anthropic.messages.create({
    ...
    system:
      "You are Bianca-03, the Slide 2 Inspector for the H+ Analytics investor deck factory. " +
      ...
      "coherent, substantive, and fits a photo gallery / operational showcase context " +
      "for Hazelnis Retreat / Loch Sheldrake. " +
    ...
    messages: [
      {
        ...
        content: [
          ...
          },
          {
            type: "text",
            text:
              "Above is the canonical Slide 2 reference (960×540 px).\n\n" +
  ...
  if (!toolBlock) {
    logger.warn("[bianca-03] Pass 2 — no tool call; defaulting to approved", "slide-factory");
    return { status: "ok", notes: null };
  }
  ...
  logger.info(
    `[bianca-03] Pass 2 ${verdict.approved ? "APPROVED" : "BLOCKED"} — ${verdict.notes ?? "no notes"}`,
    "slide-factory",
  );
