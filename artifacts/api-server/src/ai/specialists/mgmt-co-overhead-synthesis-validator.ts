/**
 * Post-synthesis quality validator for the Overhead Specialist (Phase 2 IB
 * req #9).
 *
 ...
 * parse time.
 *
 * Note on collapsed ranges: overhead dimensions emit USD integers. The
 * `low === high` sentinel still flags genuine point estimates because no
 * honest Opus emission for an overhead band collapses to a single value
 * (the bands are wide enough across founder/expansion/institutional that
 * a degenerate range signals model-output failure, not market consensus).
 * Compensation/Funding/Revenue logic carries over verbatim.
 */

import type { OverheadSpecialistOutput } from "./mgmt-co-overhead-output-schema";
...
export function validateSynthesisOutput(
  output: OverheadSpecialistOutput,
  comparables: readonly OverheadComparableRow[],
): SynthesisValidationResult {
  // 1. All 6 expected dimension keys present by identity (not just count)
  const emittedKeys = new Set(output.dimensions.map((d) => d.key));
  const missingKeys = OVERHEAD_DIMENSION_KEYS.filter((k) => !emittedKeys.has(k));
  if (missingKeys.length > 0) {
    return {
      pass: false,
      regressReason: `Synthesis omitted dimensions: ${missingKeys.join(", ")}. Emit exactly these 6 keys: ${OVERHEAD_DIMENSION_KEYS.join(", ")}.`,
    };
  }
