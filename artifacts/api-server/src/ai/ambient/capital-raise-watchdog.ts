/**
 * capital-raise-watchdog.ts — scheduled producer of fresh raise data.
 *
 * Role: this is the missing module that makes the
 * `capital_raise_benchmarks` singleton table self-refresh on a cadence,
 * instead of waiting for an admin to click the manual "refresh" button.
 *
 * Pipeline:
 *   1. The ambient scheduler ticks (every 6h, see `scheduler.ts`).
 *   2. `runCapitalRaiseWatchdogCycle` is invoked.
 *   3. We check the cadence guard — if the last successful watchdog run
 *      was less than `CAPITAL_RAISE_WATCHDOG_CADENCE_HOURS` ago (default
 *      168h = weekly), we skip without doing any work or LLM spend.
 *   4. Otherwise we call `researchCapitalRaiseBenchmarks` (the same LLM
 *      synthesizer the manual button uses) to gather fresh ranges and
 *      evidence on SAFE caps, discount rates, tranche sizes, runway
 *      months, and dilution %.
 *   5. We enforce the N+1 evidence rule (≥3 independent sources). If the
 *      LLM was unreachable or returned a fallback / under-sourced result,
 *      we open + finalize an "aborted" audit row so the admin Analyst
 *      Tables UI shows that the watchdog ran and explicitly chose not to
 *      overwrite the table.
 *   6. On a clean run, we map the proposed ranges into a
 *      `WatchdogRaiseSnapshot` and call `applyWatchdogCapitalRaiseSnapshot`,
 *      which atomically upserts the rows AND writes a "success" audit
 *      row tagged with `userAgent="capital-raise-watchdog"`. That tag
 *      is what the admin UI uses to render watchdog runs alongside
 *      manual refreshes in the audit log.
 *
...
import {
  ...
  type ApplyWatchdogCapitalRaiseResult,
  type WatchdogRaiseObservation,
} from "../analyst-table-refresh";
import { storage } from "../../storage";
import { log } from "../../logger";

const TABLE_ID = "capital_raise_benchmarks" as const;
...
const LOG_TAG = "capital-raise-watchdog";

export interface RunCapitalRaiseWatchdogOptions {
  /** Skip the cadence guard (used by tests / on-demand admin trigger). */
  force?: boolean;
}

export type RunCapitalRaiseWatchdogResult =
  | { ran: false; reason: "cadence_skipped"; nextEligibleAt: Date }
  | {
      ran: true;
      reason: "applied" | "insufficient_evidence" | "no_observations";
      result: ApplyWatchdogCapitalRaiseResult;
      sourceCount: number;
      tokensUsed: number;
    };

function cadenceMs(): number {
  const raw = process.env.CAPITAL_RAISE_WATCHDOG_CADENCE_HOURS;
...
/**
 * Map a `researchCapitalRaiseBenchmarks` result into a watchdog snapshot.
 * Drops dimensions whose three values are all null (nothing to write).
 */
function buildObservations(research: AnalystRefreshResult): WatchdogRaiseObservation[] {
  const obs: WatchdogRaiseObservation[] = [];
 ...
 */
export async function runCapitalRaiseWatchdogCycle(
  opts: RunCapitalRaiseWatchdogOptions = {},
): Promise<RunCapitalRaiseWatchdogResult> {
  const now = new Date();
  ...
  // ── Synthesize fresh raise data via the LLM ────────────────────
  // We feed in the current rows so the LLM keeps dimension keys/labels stable.
  let current: Awaited<ReturnType<typeof storage.getCapitalRaiseBenchmarks>>;
  try {
    current = await storage.getCapitalRaiseBenchmarks();
  } catch (err: unknown) {
    log(
      `Could not load current benchmarks (treating as empty): ${err instanceof Error ? err.message : String(err)}`,
      LOG_TAG,
      "warn",
    );
    current = [];
  ...
  const research = await researchCapitalRaiseBenchmarks(current);
  ...
  if (observations.length === 0) {
    const result = await applyWatchdogCapitalRaiseSnapshot({
      ...
      recordedAt: now,
      evidence: research.evidence,
      notes: "Aborted: LLM returned no usable dimension ranges.",
    });
    log(
      `Aborted — no usable dimensions in LLM response; audit row #${result.auditId ?? "?"}`,
      LOG_TAG,
      "warn",
    );
    return {
  ...
  const result = await applyWatchdogCapitalRaiseSnapshot({
    ...
    recordedAt: now,
    evidence: research.evidence,
    notes: `Scheduled watchdog refresh (${observations.length} dimension(s), ${evidenceCount} source(s), ${research.tokensUsed} tokens).`,
  });

  log(
    `Applied ${result.appliedDimensions.length} dimension(s) (skipped ${result.skippedDimensions.length}); audit row #${result.auditId ?? "?"}`,
    LOG_TAG,
  );
