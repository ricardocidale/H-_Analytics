...
8. **Cost of Equity (WACC Input)**: Recommend a cost of equity (required equity return) for private hospitality investments in this asset class. This is used as the Re component in WACC = (E/V × Re) + (D/V × Rd × (1−T)). For private companies, CAPM is not used — instead derive Re using a build-up approach: (a) start from the current USD 10-year Treasury yield (Rf, ~4.3–4.5% as of May 2026 per FRED); (b) add the Damodaran boutique hospitality sector equity risk premium (~12% for lodging; source: https://pages.stern.nyu.edu/~adamodar/New_Home_Page/datafile/wacc.html); (c) add the private-market illiquidity premium of 3–5% for boutique hospitality (Duff & Phelps / Kroll Cost of Capital Navigator 2024); (d) for non-US USD-denominated properties, add the Damodaran country risk premium (source: https://pages.stern.nyu.edu/~adamodar/New_Home_Page/datafile/ctryprem.html). Current defensible range: 18%–28% for private boutique hospitality in the current rate environment (10yr Treasury ~4.3% + sector ERP ~12% + illiquidity premium 3–5% + country risk premium as applicable). Include this as a "costOfEquity" section in your response with recommendedRate (e.g. "20%–24%"), rationale (show the build-up components), riskFactors (array of factors that push rate higher or lower), countryRiskPremium (for non-US properties, the Damodaran CRP to add to the base rate), and comparables (e.g. "Private boutique hotel equity returns average 20–24% in US secondary markets, 2024–2026 rate environment").
