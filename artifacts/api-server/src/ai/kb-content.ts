...
DSCR (Debt Service Coverage Ratio): NOI / Annual Debt Service. Must exceed 1.25x for most commercial loans. Below 1.0x indicates the property cannot cover its debt obligations from operations.`,
  ...
  chunks.push({
    title: "L+B Slide Deck Export",
    content: `The platform generates branded L+B property slide decks for investor presentations and LP packages.

Access: Admin → Slide Decks tab (admin-only feature). Each property in the portfolio gets its own slide deck row with independent download buttons.

Two formats are available:

Editable PPTX (Track 1):
- Generated by a Python pipeline server-side
- Produces a fully editable .pptx file matching the L+B template — shapes, text boxes, and images are all individually editable in PowerPoint or Keynote
- Best for: deal teams who need to customize slides for a specific LP or presentation context
- Format label: "Editable PPTX"

Image-Locked PPTX (Track 2):
- Generated entirely in Node.js using satori (JSX→SVG) and sharp (SVG→PNG), then packaged with pptxgenjs
- Each slide is a full-page image inside the PPTX — pixel-perfect, no headless browser needed
- The visual design exactly matches the L+B brand: dark green (#1C2B1E) background, sage green accent (#257D41), cream text (#FFF9F5), EB Garamond headers, Poppins body
- Best for: sending to LPs as a polished, tamper-proof deck
- Format label: "Image Deck"

Generation workflow: Files are generated on demand or pre-generated at startup and stored in cloud object storage (R2). The admin can regenerate any format for any property at any time. Each format tracks its own status badge (Generating, Ready, Error) independently.

The slide content is driven by the property's financial model — NOI, RevPAR, ADR, occupancy, IRR, equity multiple, acquisition price, and AI-generated property vision text.`,
    source: "Platform Guide",
    category: "guide",
  });
  ...
  chunks.push({
    ...
    content: `Scenarios can be shared between users on the platform.
...
Shared scenarios let the team build and compare different capital structures, stress cases, and market assumptions without duplicating properties or losing the base case.`,
    source: "Platform Guide",
    category: "guide",
  });
