...
Investor PDF:
- Rendered on demand by headless Chromium (Playwright) navigating the internal React deck route
- Pixel-locked single PDF, fonts and images embedded — best for sending to LPs as a polished, tamper-proof deck
- Format label: "Investor PDF"

Generation workflow: PPTX is pre-generated at startup and stored in cloud object storage (R2). PDF renders on demand and is cached in R2 until the property or its financials change. The admin can regenerate either format from the Slide Decks tab.
