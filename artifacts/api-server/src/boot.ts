/**
 * server/boot.ts — Async startup sequence (Phases 1–4)
 *
 * Contains the full boot sequence invoked after the Express app and middleware
 * chain are wired up in index.ts. Exports a single entry-point:
 *
 *   runBootSequence(httpServer, app)
 *
 * Phases:
 *   1. Env-var checks, storage provider init, route registration, static serving
 ...
 *   3.  Ambient schedulers (benchmark, Pietro, research, health, constants, etc.)
 *   4.  Periodic interval jobs (market rates, session cleanup, MI cache, etc.)
 *
 * Helper functions (runSchemaMigrations, runSeeds, etc.) live here too so that
 * index.ts is a pure wiring file and this file is the sole owner of boot logic.
 */
...
import { setupSentryExpressErrorHandler } from "./sentry";
import { hasDbUrl } from "@shared/db-url";
import { seedAdminUser, cleanupRateLimitMaps } from "./auth";
...
/**
 * Runs the full async startup sequence. Called once from index.ts after the
 * middleware chain is wired. Performs Phase 1 route registration synchronously
 * (awaiting storage init and route mounting), then opens the HTTP port and
 * fires off Phases 2–4 inside the listen callback.
 */
export async function runBootSequence(httpServer: Server, app: Express): Promise<void> {
