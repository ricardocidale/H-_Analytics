/**
 * check-selective.ts
 *
 * Selective check driver for `pnpm run check`.
 *
 * Instead of running ALL checks in parallel when the composite cache misses,
 * this driver pre-probes each individual check's input hash and only spawns
 * processes for checks whose inputs have actually changed since the last
 * successful run.  Unchanged checks are skipped immediately with a one-line
 * "cached" message — no process is spawned at all.  Stale checks are run in
 * parallel, preserving the concurrency of the old `check:core` script.
 *
 * HOW IT FITS INTO THE PIPELINE
 *   pnpm run check
 *     = check:all-cached                          (composite fast-path: all green → skip)
 *     || { check:selective && check:all-cached:write }
 *
 * INPUT COVERAGE
 *   This file mirrors the input-collection logic from each individual check
 *   script.  When an individual check's inputs change, update the matching
 *   collectInputs_* function here too, or the per-check cache probe will
 *   use stale inputs and incorrectly report a hit.
 *
 * TYPECHECK
 *   `typecheck` has no check-cache.ts-managed hash — it relies on tsc's own
 *   incremental state (.tsbuildinfo files).  It is always queued as stale so
 *   tsc's own caching provides the skip.
 *
 * OUTPUT FORMAT
 *   [skip]  check:<name>  — inputs unchanged, skipping
 *   [run]   check:<name>  — inputs changed, running
 *   [pass]  check:<name>  completed in Xs
 *   [fail]  check:<name>  FAILED (exit N) after Xs
 *   [done]  N/M checks ran fresh, M-N cached — all passed.
 *
 * BYPASS
 *   CHECK_CACHE_DISABLED=1 — treat every check as stale (same as the
 *   individual checks honour this flag).
 */

import { spawn } from "node:child_process";
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

import {
  computeInputsHash,
  walkFilesForCache,
  WORKSPACE_ROOT,
} from "./lib/check-cache.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

const CACHE_DIR = path.join(WORKSPACE_ROOT, ".cache");

function readStoredHash(name: string): string | null {
  try {
    return fs.readFileSync(path.join(CACHE_DIR, `check-${name}.hash`), "utf8").trim() || null;
  } catch {
    return null;
  }
}

function isStale(name: string, freshHash: string): boolean {
  if (process.env.CHECK_CACHE_DISABLED === "1") return true;
  return readStoredHash(name) !== freshHash;
}

// ---------------------------------------------------------------------------
// Per-check input-collection functions
// Mirror the logic from each individual check script exactly.
// Update here whenever a check script's collectInputFiles() changes.
// ---------------------------------------------------------------------------

const SOURCE_EXTS = new Set([".ts", ".tsx", ".js", ".jsx", ".mjs", ".mts"]);

const COMMON_SKIP = new Set([
  "node_modules", ".git", ".cache", "dist", "build", "__generated__", "worktrees",
]);

function collectInputs_lint(): string[] {
  const files: string[] = [
    path.join(__dirname, "check-lint.ts"),
  ];

  const SOURCE_DIRS = [
    path.join(WORKSPACE_ROOT, "lib"),
    path.join(WORKSPACE_ROOT, "artifacts"),
    path.join(WORKSPACE_ROOT, "scripts", "src"),
  ];
  const SKIP = new Set([
    ...COMMON_SKIP, "migrations", ".claude",
  ]);

  for (const dir of SOURCE_DIRS) {
    for (const f of walkFilesForCache(dir, { extensions: SOURCE_EXTS, skipDirs: SKIP })) {
      files.push(f);
    }
  }

  for (const cfg of [
    path.join(WORKSPACE_ROOT, "artifacts/api-server/eslint.config.mjs"),
    path.join(WORKSPACE_ROOT, "artifacts/hospitality-business-portal/eslint.config.mjs"),
    path.join(WORKSPACE_ROOT, "lib/shared/eslint.config.mjs"),
  ]) {
    if (fs.existsSync(cfg)) files.push(cfg);
  }

  return files;
}

function collectInputs_lintLibs(): string[] {
  const files: string[] = [
    path.join(__dirname, "check-lint-libs.ts"),
  ];
  const SKIP = new Set([...COMMON_SKIP, ".claude"]);

  for (const f of walkFilesForCache(path.join(WORKSPACE_ROOT, "lib"), {
    extensions: SOURCE_EXTS,
    skipDirs: SKIP,
  })) {
    files.push(f);
  }

  const cfg = path.join(WORKSPACE_ROOT, "lib/shared/eslint.config.mjs");
  if (fs.existsSync(cfg)) files.push(cfg);

  return files;
}

function collectInputs_magicNumbers(): string[] {
  const files: string[] = [
    path.join(__dirname, "check-magic-numbers.ts"),
    path.join(__dirname, "_magic-numbers-baseline.json"),
  ];

  const SCAN_DIRS = [
    "lib/calc/src",
    "lib/engine/src",
    "lib/shared/src",
    "lib/domain/src",
    "lib/analytics/src",
  ];
  const SKIP = new Set([
    "node_modules", ".git", ".cache", ".claude", "dist", "build", "__generated__",
  ]);
  const SKIP_SUFFIXES = new Set([
    ".test.ts", ".test.tsx", ".spec.ts", ".spec.tsx",
    "_render-harness.ts", "smoke-producer.ts",
  ]);

  function walkMagicNumbers(dir: string): void {
    let entries: fs.Dirent[];
    try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch { return; }
    for (const entry of entries) {
      if (entry.isDirectory()) {
        if (!SKIP.has(entry.name)) walkMagicNumbers(path.join(dir, entry.name));
      } else if (entry.isFile()) {
        const ext = path.extname(entry.name);
        if (ext !== ".ts" && ext !== ".tsx") continue;
        if ([...SKIP_SUFFIXES].some(s => entry.name.endsWith(s))) continue;
        files.push(path.join(dir, entry.name));
      }
    }
  }

  for (const dir of SCAN_DIRS) {
    walkMagicNumbers(path.join(WORKSPACE_ROOT, dir));
  }

  // Server directory (artifacts/api-server/src, excluding migrations)
  const serverDir = path.join(WORKSPACE_ROOT, "artifacts/api-server/src");
  const serverSkip = new Set([...SKIP, "migrations"]);
  function walkServer(dir: string): void {
    let entries: fs.Dirent[];
    try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch { return; }
    for (const entry of entries) {
      if (entry.isDirectory()) {
        if (!serverSkip.has(entry.name)) walkServer(path.join(dir, entry.name));
      } else if (entry.isFile()) {
        const ext = path.extname(entry.name);
        if (ext !== ".ts" && ext !== ".tsx") continue;
        if ([...SKIP_SUFFIXES].some(s => entry.name.endsWith(s))) continue;
        files.push(path.join(dir, entry.name));
      }
    }
  }
  walkServer(serverDir);

  return files;
}

function collectInputs_replitIndependence(): string[] {
  const CHECKED_EXTS = new Set([".ts", ".tsx", ".js", ".mjs", ".cjs"]);
  const SKIP = new Set([
    "node_modules", ".git", ".cache", ".claude", "dist", "build", ".local",
    "vendor", "attached_assets", "screenshots", "tmp",
  ]);

  const files: string[] = [
    path.join(__dirname, "check-replit-independence.ts"),
  ];

  function* walk(dir: string): Generator<string> {
    let entries: fs.Dirent[];
    try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch { return; }
    for (const entry of entries) {
      if (entry.isDirectory()) {
        if (!SKIP.has(entry.name)) yield* walk(path.join(dir, entry.name));
      } else if (entry.isFile()) {
        if (CHECKED_EXTS.has(path.extname(entry.name))) {
          yield path.join(dir, entry.name);
        }
      }
    }
  }

  for (const f of walk(WORKSPACE_ROOT)) {
    files.push(f);
  }

  return files;
}

function collectInputs_migrationGuards(): string[] {
  const JOURNAL = path.join(
    WORKSPACE_ROOT, "artifacts/api-server/migrations/meta/_journal.json",
  );
  const GUARDS_DIR = path.join(WORKSPACE_ROOT, "artifacts/api-server/src/migrations");
  const MANIFEST = path.join(GUARDS_DIR, "migration-guards.json");

  const files: string[] = [
    path.join(__dirname, "check-migration-guards.ts"),
    JOURNAL,
    MANIFEST,
  ];

  try {
    for (const entry of fs.readdirSync(GUARDS_DIR, { withFileTypes: true })) {
      if (!entry.isFile()) continue;
      if (entry.name.endsWith(".ts") || entry.name.endsWith(".json")) {
        files.push(path.join(GUARDS_DIR, entry.name));
      }
    }
  } catch {
    // directory missing — the check itself will surface the error
  }

  return files;
}

function collectInputs_spinnerContrast(): string[] {
  const files: string[] = [
    path.join(__dirname, "check-spinner-contrast.ts"),
  ];

  const SCAN_DIRS = ["artifacts/hospitality-business-portal/src"];

  // Mirror check-spinner-contrast.ts walkFiles exactly:
  // only .tsx files, with its specific SKIP_DIRS set.
  const SKIP = new Set([
    "node_modules", ".git", ".cache", ".claude", "dist", "build",
    ".local", "vendor", "attached_assets", "screenshots", "tmp",
  ]);

  function* walkTsx(dir: string): Generator<string> {
    let entries: fs.Dirent[];
    try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch { return; }
    for (const entry of entries) {
      if (entry.isDirectory()) {
        if (!SKIP.has(entry.name)) yield* walkTsx(path.join(dir, entry.name));
      } else if (entry.isFile() && entry.name.endsWith(".tsx")) {
        yield path.join(dir, entry.name);
      }
    }
  }

  for (const scanDir of SCAN_DIRS) {
    const absDir = path.join(WORKSPACE_ROOT, scanDir);
    if (!fs.existsSync(absDir)) continue;
    for (const f of walkTsx(absDir)) files.push(f);
  }

  return files;
}

function collectInputs_productionImage(): string[] {
  const SRC_SKIP = new Set(["node_modules", "dist", ".cache"]);

  const APPS = [
    { srcDist: "artifacts/hospitality-business-portal/dist/public" },
    { srcDist: "artifacts/mockup-sandbox/dist" },
  ];

  const files: string[] = [
    path.join(__dirname, "check-production-image.ts"),
    path.join(WORKSPACE_ROOT, "Dockerfile"),
    path.join(WORKSPACE_ROOT, "pnpm-lock.yaml"),
  ];

  const ROOT_CONFIG_FILES = ["package.json", "index.html", "vite.config.ts", "vite.config.js"];

  function* walkAllFiles(dir: string): Generator<string> {
    let entries: fs.Dirent[];
    try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch { return; }
    for (const entry of entries) {
      if (entry.isDirectory()) {
        if (!SRC_SKIP.has(entry.name)) yield* walkAllFiles(path.join(dir, entry.name));
      } else if (entry.isFile()) {
        yield path.join(dir, entry.name);
      }
    }
  }

  for (const app of APPS) {
    const artifactRootRel = app.srcDist.split("/").slice(0, 2).join("/");
    const artifactRoot = path.join(WORKSPACE_ROOT, artifactRootRel);

    for (const name of ROOT_CONFIG_FILES) {
      const candidate = path.join(artifactRoot, name);
      if (fs.existsSync(candidate)) files.push(candidate);
    }

    try {
      for (const name of fs.readdirSync(artifactRoot)) {
        if (/^tsconfig.*\.json$/.test(name)) {
          files.push(path.join(artifactRoot, name));
        }
      }
    } catch { /* artifact root missing */ }

    const srcDir = path.join(artifactRoot, "src");
    if (fs.existsSync(srcDir)) {
      for (const f of walkAllFiles(srcDir)) files.push(f);
    }
  }

  return files;
}

function collectInputs_typesMirror(): string[] {
  const REPO_ROOT = path.resolve(__dirname, "..", "..");
  return [
    path.join(__dirname, "check-types-mirror.ts"),
    path.join(REPO_ROOT, "artifacts/api-server/src/slides/types.ts"),
    path.join(
      REPO_ROOT,
      "artifacts/hospitality-business-portal/src/features/internal-deck/types.ts",
    ),
  ];
}

function collectInputs_schemaDrift(): string[] {
  const DB_LIB = path.join(WORKSPACE_ROOT, "lib/db");
  const files: string[] = [
    path.join(__dirname, "check-schema-drift.ts"),
  ];

  for (const f of walkFilesForCache(path.join(DB_LIB, "src"), {
    extensions: new Set([".ts"]),
    skipDirs: new Set(["node_modules"]),
  })) {
    files.push(f);
  }

  for (const f of walkFilesForCache(path.join(DB_LIB, "migrations"), {
    extensions: new Set([".sql", ".json"]),
    skipDirs: new Set(["node_modules"]),
  })) {
    files.push(f);
  }

  return files;
}

// ---------------------------------------------------------------------------
// Check registry
// ---------------------------------------------------------------------------

interface CheckSpec {
  /** Cache name — matches the key used by the individual check (check-<name>.hash). */
  name: string;
  /** Human-readable label used in log output. */
  label: string;
  /** Root package.json script to run when stale. */
  rootScript: string;
  /** Collect the full list of input file paths for this check. */
  collectInputs: () => string[];
  /** Optional extra string folded into the hash (e.g. CLI args). */
  extra?: string;
}

const SCRIPT_CHECKS: CheckSpec[] = [
  {
    name: "lint",
    label: "lint",
    rootScript: "check:lint",
    collectInputs: collectInputs_lint,
  },
  {
    name: "lint-libs",
    label: "lint:libs",
    rootScript: "check:lint:libs",
    collectInputs: collectInputs_lintLibs,
  },
  {
    name: "magic-numbers",
    label: "magic-numbers",
    rootScript: "check:magic-numbers",
    collectInputs: collectInputs_magicNumbers,
    extra: "threshold=4",
  },
  {
    name: "replit-independence",
    label: "replit-independence",
    rootScript: "check:replit-independence",
    collectInputs: collectInputs_replitIndependence,
  },
  {
    name: "migration-guards",
    label: "migration-guards",
    rootScript: "check:migration-guards",
    collectInputs: collectInputs_migrationGuards,
  },
  {
    name: "spinner-contrast",
    label: "spinner-contrast",
    rootScript: "check:spinner-contrast",
    collectInputs: collectInputs_spinnerContrast,
  },
  {
    name: "production-image",
    label: "production-image",
    rootScript: "check:production-image",
    collectInputs: collectInputs_productionImage,
  },
  {
    name: "types-mirror",
    label: "types-mirror",
    rootScript: "check:types-mirror",
    collectInputs: collectInputs_typesMirror,
  },
  {
    name: "schema-drift",
    label: "schema-drift",
    rootScript: "check:schema-drift",
    collectInputs: collectInputs_schemaDrift,
  },
];

/**
 * typecheck has no check-cache.ts hash — it relies on tsc incremental state.
 * Always queue it; tsc itself decides whether to recheck.
 */
const TYPECHECK_SCRIPT = "typecheck";
const TYPECHECK_LABEL = "typecheck";

// ---------------------------------------------------------------------------
// Probe: compute fresh hash and compare to stored
// ---------------------------------------------------------------------------

interface ProbeResult {
  spec: CheckSpec;
  freshHash: string;
  stale: boolean;
}

function probeAll(): ProbeResult[] {
  return SCRIPT_CHECKS.map((spec) => {
    const freshHash = computeInputsHash({
      files: spec.collectInputs(),
      extra: spec.extra,
    });
    return { spec, freshHash, stale: isStale(spec.name, freshHash) };
  });
}

// ---------------------------------------------------------------------------
// Run a single check as a child process, streaming its output
// ---------------------------------------------------------------------------

interface RunResult {
  label: string;
  exitCode: number;
  durationMs: number;
  /** True when the process was killed because another check failed first. */
  killed?: boolean;
}

/**
 * Run one check as a child process, streaming its output.
 * If `signal` is already aborted when the process starts, or aborts while the
 * process is running, the child receives SIGTERM (then SIGKILL after 3 s).
 */
function runCheck(
  rootScript: string,
  label: string,
  signal: AbortSignal,
): Promise<RunResult> {
  return new Promise((resolve) => {
    const start = Date.now();

    if (signal.aborted) {
      resolve({ label, exitCode: 130, durationMs: 0, killed: true });
      return;
    }

    const child = spawn(
      "pnpm",
      ["run", rootScript],
      {
        cwd: WORKSPACE_ROOT,
        stdio: "inherit",
        shell: false,
        env: process.env,
      },
    );

    let killedBySignal = false;

    function killChild(): void {
      killedBySignal = true;
      child.kill("SIGTERM");
      // Escalate to SIGKILL after 3 s if the process hasn't exited.
      setTimeout(() => {
        try { child.kill("SIGKILL"); } catch { /* already gone */ }
      }, 3000).unref();
    }

    signal.addEventListener("abort", killChild, { once: true });

    child.on("close", (code) => {
      signal.removeEventListener("abort", killChild);
      resolve({
        label,
        exitCode: code ?? 1,
        durationMs: Date.now() - start,
        killed: killedBySignal,
      });
    });

    child.on("error", (err) => {
      signal.removeEventListener("abort", killChild);
      console.error(`[check-selective] failed to spawn ${label}: ${err.message}`);
      resolve({ label, exitCode: 1, durationMs: Date.now() - start });
    });
  });
}

// ---------------------------------------------------------------------------
// Main
// ---------------------------------------------------------------------------

async function main(): Promise<void> {
  // 1. Probe all script-based checks.
  const probes = probeAll();

  const stale = probes.filter((p) => p.stale);
  const cached = probes.filter((p) => !p.stale);

  // 2. Report cached checks immediately.
  for (const p of cached) {
    console.log(`[skip]  check:${p.spec.label.padEnd(22)} inputs unchanged`);
  }

  // 3. Report which checks will run.
  const toRun: Array<{ rootScript: string; label: string }> = [];

  for (const p of stale) {
    console.log(`[run]   check:${p.spec.label.padEnd(22)} inputs changed`);
    toRun.push({ rootScript: p.spec.rootScript, label: p.spec.label });
  }

  // typecheck always runs — tsc manages its own incremental cache (.tsbuildinfo)
  // and exits near-instantly when nothing has changed.
  console.log(`[run]   check:${TYPECHECK_LABEL.padEnd(22)} always queued (tsc incremental)`);
  toRun.push({ rootScript: TYPECHECK_SCRIPT, label: TYPECHECK_LABEL });

  if (toRun.length === 0) {
    // Edge case: all script checks cached and typecheck somehow not queued
    console.log("[done]  all checks cached — nothing to run");
    process.exit(0);
  }

  console.log("");

  // 4. Run stale checks in parallel with fail-fast cancellation.
  // When any check fails, an AbortController signals siblings to terminate,
  // matching the behaviour of the old `concurrently --kill-others-on-fail`.
  const controller = new AbortController();
  const { signal } = controller;

  // Track first failure so we can abort siblings immediately.
  let firstFailure: RunResult | null = null;

  const promises = toRun.map(({ rootScript, label }) =>
    runCheck(rootScript, label, signal).then((result) => {
      if (result.exitCode !== 0 && !result.killed && firstFailure === null) {
        firstFailure = result;
        controller.abort();
      }
      return result;
    }),
  );

  const results = await Promise.all(promises);

  console.log("");

  // 5. Summary.
  const actualFailures = results.filter((r) => r.exitCode !== 0 && !r.killed);
  const killed = results.filter((r) => r.killed);
  const passes = results.filter((r) => r.exitCode === 0);

  for (const r of passes) {
    const secs = (r.durationMs / 1000).toFixed(1);
    console.log(`[pass]  check:${r.label.padEnd(22)} completed in ${secs}s`);
  }
  for (const r of actualFailures) {
    const secs = (r.durationMs / 1000).toFixed(1);
    console.error(`[fail]  check:${r.label.padEnd(22)} FAILED (exit ${r.exitCode}) after ${secs}s`);
  }
  for (const r of killed) {
    console.error(`[kill]  check:${r.label.padEnd(22)} terminated (another check failed)`);
  }

  console.log("");
  const ranCount = toRun.length;
  const skippedCount = cached.length;
  const totalCount = SCRIPT_CHECKS.length + 1; // +1 for typecheck

  if (actualFailures.length === 0) {
    console.log(
      `[done]  ${ranCount}/${totalCount} checks ran fresh` +
        (skippedCount > 0 ? `, ${skippedCount} skipped (cached)` : "") +
        " — all passed.",
    );
    process.exit(0);
  } else {
    console.error(
      `[done]  ${actualFailures.length} check(s) failed` +
        (skippedCount > 0 ? ` (${skippedCount} cached checks were skipped)` : "") +
        ".",
    );
    process.exit(1);
  }
}

main().catch((err) => {
  console.error("[check-selective] unexpected error:", err);
  process.exit(1);
});
