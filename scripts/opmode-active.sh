...
# operating-mode toggle (Task #1385 / #1386).
#
...
opmode_repo_root() {
  # Locate repo root from this script's location, falling back to PWD.
  local self_dir
...
# When executed directly (not sourced), exit with the toggle status so callers
# can use `if scripts/opmode-active.sh; then ...; fi`.
