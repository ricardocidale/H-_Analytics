...
import type {
  BracketMixEntry,
  IcpBracketProfile,
} from "../company/icp-bracket-types";
 ...
 * "no bracket scaling".
 *
 * Exported as both `normalizeBracketMix` (frontend-facing) and
 * `normalizePersistedBracketMix` (legacy server-side name).
 */
export function normalizeBracketMix(raw: unknown): NormalizedBracketMix | null {
...
/** @alias normalizeBracketMix — kept for server-side callers. */
export const normalizePersistedBracketMix = normalizeBracketMix;
