...
| 2026-05-17 | **Save button audit plan + skill updates.** Plan doc at `docs/plans/save-button-audit-2026-05-17.md`. Skills updated: `hplus-assumption-lifecycle` (Save must not navigate away; `useUnsavedExitGuard`/`UnsavedExitDialog` as canonical exit pattern); `hplus-form-actions` (two-dialog taxonomy: `AnalystUnsavedChangesDialog` for Analyst→dirty, `UnsavedExitDialog` for nav-exit). Design decisions captured: PropertyEdit stays on page after save; BracketMix validation-gate preserved; ModelDefaults guard fires on tab switch + section switch + route leave. |
