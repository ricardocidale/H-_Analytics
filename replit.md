...
- **Task #400 — Constants registry sweep (Audit #319 R4):** migrate the 7 duplicated keys (`taxRate`, `depreciationYears`, `inflationRate`, `costRateTaxes`, `countryRiskPremium`, `capitalGainsRate`, `daysPerMonth`) from hardcoded exports in `shared/constants.ts` + `shared/countryDefaults.ts` to `getFactoryValue()` reads against `shared/model-constants-registry.ts`. Legacy exports stay as fallbacks but get `@deprecated`. Closes C1/C2/H2/H4. Plan: `.local/tasks/audit-319-r4-registry-sweep.md`. **DONE (April 22, 2026):** `getFactoryNumber(key, country?, subdivision?)` typed accessor (compile-time-checked against `RegisteredConstantKey = keyof typeof MODEL_CONSTANTS_REGISTRY`) added to `shared/model-constants-registry.ts`; engine/calc/server/client consumers now route through it. The two intentionally-divergent legacy fallbacks (`DEFAULT_COMPANY_TAX_RATE` 0.30 vs registry US `taxRate` 0.21; `DEFAULT_COST_RATE_TAXES` 0.03 vs registry US `costRateTaxes` 0.012) are preserved as admin/UI fallbacks per their `@deprecated` docstrings, and the regression test `tests/server/model-constants-registry-flow.test.ts` (10 cases) asserts BOTH static parity for the 5 safe keys AND end-to-end apply-proposal HTTP flow → resolver pickup, AND the deliberate divergence so the next audit pass is a conscious reconciliation.
