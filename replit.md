...
- **Inflation rate is the cascade exception (CRITICAL).** Inflation is **not** a depreciation-style regulatory constant. Engine cascade is `property.inflationRate ?? companyAssumptions.inflationRate ?? marketMacroFallback` — the **Management Company assumptions row is the source of truth** and the per-property field is the override. Defaults Admin (Market & Macro tab) holds inflation only as a seed value and last-resort fallback. The Constants table **may** hold inflation rows, but only when the row carries a monetary-authority citation (US Federal Reserve target, IMF WEO, central-bank target) **and** was written by an AI Intelligence specialist — never by admin hand-entry. Specialists in the AI Intelligence realm are the writers; humans (admins, founders, users) are passive consumers of those suggestions. A hard-coded `const INFLATION_RATE = 0.03` anywhere in `calc/`/`engine/`/`server/`/`client/` is a defect. Adding `inflationRate` to `COUNTRY_KEYS_OVERLAID_ON_GLOBAL` requires specialist-sourced canonical rows + production-deviation backfill + the behavior-preservation guard, all three. Full rule: `.claude/rules/inflation-cascade.md`.
