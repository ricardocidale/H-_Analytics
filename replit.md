...
| Date | Change |
|---|---|
| 2026-05-17 | **UI canonical enforcement gate shipped (Plan 2026-05-16-004; CLAUDE.md §13).** Mechanical zero-tolerance gate added at `scripts/src/check-ui-canonical.ts` covering Rule A (canonical `Analyst` CTA text + identifiers + `<AnalystActionButton label>` JSX prop with multi-line buffer) and Rule B (canonical `<CurrentThemeTab>` wrapper; bare `TabsList`/`TabsTrigger` imports outside `tabs.tsx` forbidden; hand-rolled `<button>+activeTab===` heuristic). Cleanup unit: 5 files for Rule A (`FilterBar`, `ReferenceRangesTab`, `PropertyUnderwritingTab`, `CompanyAssumptionsTabsView`, `Section16Admin`); 12 files for Rule B (`PropertyDetailDrawer` + 11 gray-zone pages/features). `CurrentThemeTab` rebuilt on Radix internals (`TabsPrimitive.List`/`Trigger` + internal Root), gaining `role="tab"`/`aria-selected`/arrow-key nav; new affordances added — `suffix`, `trailingIcon`, `disabled`+`tooltipTitle`, `responsive: { fallback: "select" }`, `variant: "default" \| "drawer"`. Meta-checker `check:gate-health` registers gates and asserts file-exists / CI-wired / effective; initial registry: `check:ui-canonical`. CI wiring: `.github/workflows/ci.yml` gains "UI canonical gate" + "Gate health check" steps. Gates: typecheck ✅, check:ui-canonical ✅, check:gate-health ✅, scripts vitest ✅ (17/17 new tests). |
| 2026-05-17 | **Animations moved to AI Intelligence / Knowledge & Resources (Plan 2026-05-16-003).** `AnimationsTab` removed from Admin Brand Assets. New `AnimationsPage` under Intelligence with two tabs — "Rebecca" (7 animation cards) and "The Analyst" (8 cards). 7 new animation components: `RebeccaAliveGeometry` (12-instance merged Lascaux + geo orbital), `AnalystBarChartPulse`, `AnalystExpandingSolver`, `AnalystNexusCore`, `AnalystQuantumSolver`, `AnalystSwissCube`, `AnalystThinkingCube`. `IntelligenceSidebar` and `Intelligence.tsx` wired for `"animations"` section with `IconSparkles`. Pre-existing TS error in `RebeccaAdvancedOrbit` fixed (`TargetAndTransition` / `Transition` types). Gates: typecheck ✅, portal lint ✅, production-image ✅, flex-label-overflow ✅, spinner-contrast ✅. |
