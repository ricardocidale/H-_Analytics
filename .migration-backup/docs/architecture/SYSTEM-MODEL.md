...
**N1. ✅ OT-A.3 + OT-A.4 shipped (April 20, 2026).** The migration arc from custom JSON parsing to Vercel AI SDK structured output is complete. Commit `7da9f25a` retires `server/ai/research-value-extractor.ts`, flips `USE_AI_SDK_SYNTHESIS=true` by default, introduces a `synthesisOutputToLegacyJson` adapter in `synthesis-schema.ts` (the common envelope that `extractGuidance` + UI + single-model fallback all consume), and bumps `ENGINE_VERSION` v1→v2. The journey catalogued **four distinct LLM-migration mechanism bugs**, each now codified as an enforceable rule:

1. **Definition drift** — fields with mismatched unit/denominator/scope between paths. Fixed by `FIELD_DEFINITIONS` contract in `synthesis-schema.ts`.
2. **Mode collapse** — numeric typical-range hints in prompts cause Opus prescription leakage (identical outputs across diverse inputs). Rule: `.claude/rules/field-definitions-no-prescription-hints.md` + `tests/proof/field-definitions-no-hints.test.ts`.
3. **Representational mismatch** — two paths emitting different output shapes (point vs range) can't be raw-parity tested. Rule: `.claude/rules/llm-contract-migration-parity.md`.
4. **Parity-against-broken-baseline** — legacy output itself inaccurate on some fields; "regression" measures new path diverging from a wrong baseline. Rule: `.claude/rules/parity-exemption-classes.md` (four-class exemption taxonomy: industry-standard / legacy-inaccurate / noise-floor / under-reasoned).

**Post-OT-A.4 observation window:** 72 hours of production monitoring before OT-A.5 work involves any API spend. Watch Sentry for `ContractMigrationParityMisuseError`; if zero issues, authorize v6 rerun for OT-A.5 batch validation.

**OT-A.5 queued (not yet started):** promote `inflationRate` from exemption to design-fix (if legacy genuinely hard-codes USA CPI); 6 T2 USALI cost-line anchors (costHousekeeping +43% bias, costMarketing +28%, preOpeningCosts −36%, startOccupancy −37%, catering +16%, costPropertyTaxes −13%); 4 non-T1 mode-collapse fields (costFB, costSeg5yrPct, svcFeeGeneralMgmt, svcFeeTechRes). Replit drafting `OT-A-5-design.md` during the observation window; single $22 rerun at T+72h to validate all three tracks together.

**N2. Ship the verdict cache. (M, leverage 5.)**
**ADR-004 accepted** 2026-04-20 at `docs/architecture/decisions/ADR-004-verdict-cache.md`. Phase 5A Claude-side code shipped (`engine/analyst/cognitive/cache-keys.ts`, 21 tests PASS). Phase 5A migrations queued for Replit at `.claude/replit-handoffs/phase-5a-verdict-cache-migrations.md`. Decision: content-addressed cache layered over existing `research_runs` + `assumption_guidance` (no new tables). Two-axis TTL (time + `inputContextHash`); automatic invalidation on property/global mutation and pgvector reindex; miss path is stream-through with write-after. Remaining phases: 5A migrations (Replit, queued) → 5B façade read path (Claude, blocked on 5A) → 5C write-after + invalidation (Replit) → 5D observability (Replit, pairs with PostHog handoff). Why top: turns ~$125/day into ~$25/day at current volume and unlocks ambient/cross-portfolio UX patterns that are cost-prohibitive today.
...
**N6b. Workspace reorganization (ADR-005). (L, leverage 3.)** Proposed 2026-04-20 at `docs/architecture/decisions/ADR-005-workspace-reorganization.md`; 4 open questions resolved, Phase 1 handoff queued at `.claude/replit-handoffs/phase-1-workspace-bootstrap.md`. Stays Proposed until Phase 1 (PNPM + Turborepo bootstrap, zero file moves) and Phase 2 (`packages/shared` extraction) both land cleanly. Expected payoff: ~40–60% CI time reduction on UI PRs; selective OSS of `packages/calc`; package-level ownership narrows cross-agent collisions. Scope: 8 phases over ~2 weeks; Claude plans, Replit executes. Not urgent — execute during a calm window, not alongside OT-A.5 or active ADR-004 implementation.
