...
const FORBIDDEN_LITERALS: Record<string, string> = {
  ...
  "0.25": "DEFAULT_PROPERTY_INCOME_TAX_RATE / DEFAULT_LAND_VALUE_PERCENT",
  "39": "DEPRECIATION_YEARS",
  "30.5": "DAYS_PER_MONTH",
  "2500000": "DEFAULT_CAPITAL_RAISE_VALUATION_CAP",
...
// Zero violations — baseline is empty. Any new hardcoded number fails CI immediately.
const KNOWN_MAGIC_NUMBER_BASELINE = new Set<string>([]);

// Zero violations — baseline is empty. Any new hardcoded number fails CI immediately.
const KNOWN_FORBIDDEN_BASELINE = new Set<string>([]);
  ...
  describe("Coverage check — all runtime financial files are scanned", () => {
    it("no unscanned .ts files in client/src/lib/ with financial calculations", () => {
      ...
      const nonFinanceFiles = new Set([
        ...
        "specialist-nav.ts",
        "analyst-focus-field.ts",
        "analyst-mount-points.ts",
      ]);
