...
Adds a Knowledge Registry section to AI Intelligence: a `knowledge_registry` DB table seeded with 8 assets, API routes for listing and regenerating those assets, and two new frontend pages (the registry overview with type-specific content viewers and an Analyst regeneration button per asset; plus a Country Economic Data sub-page). Implements the control-plane described in §§4–9 of the origin requirements doc.
...
- R4. The Knowledge Registry page in AI Intelligence renders one collapsible asset panel per registry entry, collapsed by default, showing name, freshness badge, chunk/row count, and Analyst button.
- R5. Each asset panel's expanded state shows a type-specific content viewer: text-chunk cards (vector_namespace), ranges grid (benchmark_table), brands card grid (benchmark_brands), or link-out (country_data).
...
- **No per-asset regeneration queuing in v1** — POST /:id/regenerate is synchronous (SSE-streamed via AnalystRefreshTheater, same as AnalystTables). A long-running regeneration holds the connection. Post-v1 work can add a job queue.
...
- **Exactly which Analyst specialist(s) to call per vector namespace** during regeneration — the routing table (namespace → specialist ID) exists in the codebase; the implementing agent should read `lib/engine/src/analyst/registry/` and match namespaces to their owner specialists.
...
Frontend (KnowledgeRegistryPage)
    ...
    │    └─ expanded: TypeSpecificViewer (chunks | ranges | brands | link-out)
...
- Do NOT seed `countryEconomicData` rows in this file — those are populated by the regeneration endpoint (US/MX/CO/BR are seeded with last-known values via the boot-gate migration pattern if pre-populating, otherwise the table starts empty and users regenerate).
...
- `POST /api/admin/knowledge-registry/:id/regenerate` — look up entry; 404 if missing. Dispatch by `asset_type`:
  - `vector_namespace`: route to the owner Specialist (read `lib/engine/src/analyst/registry/` to find the namespace→specialist mapping); on success upsert new chunks, call `updateKnowledgeRegistryRefreshed`.
  - `benchmark_table` / `benchmark_brands`: delegate to the existing benchmark regeneration path; call `updateKnowledgeRegistryRefreshed` on success.
...
- `TypeSpecificViewer` switch: `vector_namespace` → `VectorChunkViewer` (paginated list of recent chunks, 20/page, fetched from a new `GET /api/admin/knowledge-registry/:id/chunks?page=N` sub-route or existing vector-search endpoint); `benchmark_table` → render inline ranges grid; `benchmark_brands` → `<ReferenceBrandsGrid />`; `country_data` → navigate link to `knowledge-registry-country-data` section.
...
- Happy path: expanding a `country_data` panel shows a link-out to Country Economic Data
...
- **Interaction graph:** `seeds/index.ts` startup path gains a new seed call. Admin routes gain 5 new endpoints. `IntelligenceV2Storage` gains 5 new methods delegated to `KnowledgeRegistryStorage`. `AiIntelligenceSidebar` type union and nav groups change — existing sections are unaffected but the type change must be exhaustive.
...
| Vector namespace regeneration via Specialist may be slow (LLM call + embedding) | Already handled by `AnalystRefreshTheater`'s streaming SSE design — same pattern used by AnalystTables today |
