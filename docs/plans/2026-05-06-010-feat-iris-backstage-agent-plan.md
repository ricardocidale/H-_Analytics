...
- **Model tier selection is trigger-driven.** `trigger: "health-check"` → Haiku. `trigger: "manual" | "gap-signal" | "weekly-reindex"` → Sonnet. Rationale: health checks are tool-loop-heavy with simple per-tool reasoning; Sonnet is reserved for ingestion runs that require judgment about what to ingest.
