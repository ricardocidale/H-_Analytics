...
| [ ] | **BLOCKED — `DEFAULT_TRAVEL_COST_PER_CLIENT` + `DEFAULT_IT_LICENSE_PER_CLIENT`**: value conflict between constants.ts (5000/3600) and syncHelpers (12000/3000 via constants-staffing). syncHelpers bug vs. intentional? Must resolve discrepancy before retiring. See `.agents/status/cc.md` conflict notes. | T1-4 incremental |
