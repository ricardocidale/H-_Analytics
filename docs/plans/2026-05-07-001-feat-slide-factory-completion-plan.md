...
- **Error propagation:** Per-slide team failures populate `agentResults[slideN].verdict = 'block'` and surface in Tab 5 UI with red badges. Marco transitions status to `error` if any slide is unrecoverable; the existing `error_message` column on `slide_factory_runs` carries the structured note. Rebecca tools surface errors via `isError: true` per Rebecca pattern.
