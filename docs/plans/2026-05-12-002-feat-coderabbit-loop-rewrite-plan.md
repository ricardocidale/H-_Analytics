...
- R8. The slash command runs synchronously; the Claude Code session is blocked for the full loop duration. Wall-time expectation: working-tree loop (U2) is seconds to minutes; open-PR autofix loop (U3) may take 7–30+ min per iteration × up to 4 iterations. Per-iteration state is checkpointed to `.local/coderabbit-loop/` so `/coderabbit-loop-status` shows live progress and a killed session can report last-known state on re-invocation.
...
- **Synchronous foreground loop.** The slash command runs in the foreground for the full loop duration — the Claude Code session is blocked while the loop runs. Claude orchestrates each iteration directly: trigger CR, wait (polling with `sleep 90` + status output), read findings, apply fixes using Edit tools in the same session, run per-iteration gates, commit, and loop. No backgrounded watcher process; no IPC. Per-iteration state is written to `.local/coderabbit-loop/` as a progress log so `/coderabbit-loop-status` can report current state and a killed session reports last-known state on re-invocation. Wall-time expectation: working-tree loop is seconds to minutes; open-PR autofix loop may take 7–30+ min per iteration × up to 4 iterations. This matches the curl-trigger-and-poll precedent in `docs/solutions/workflow-issues/coderabbit-iterative-review-loop-on-replit-agent-2026-05-09.md` and the "start it and step away" use case makes blocking the session an acceptable tradeoff.
...
- **Watcher↔session fix-application handoff architecture — RESOLVED (2026-05-12): Synchronous loop (Option A).** R8 updated. U2/U3 approaches updated to reflect foreground orchestration by Claude. The "backgrounded watcher" framing is removed from all sections.
  - **Resume-from-checkpoint contract — RESOLVED.** With the synchronous architecture, resume is an explicit non-guarantee: each invocation starts fresh; the `.local/coderabbit-loop/run.json` state file is a diagnostic/status aid only, not a resume checkpoint. On re-invocation after a killed session, `/coderabbit-loop-status` reports last-known state, but the loop restarts from iteration 1. Scope Boundaries updated to reflect this.
