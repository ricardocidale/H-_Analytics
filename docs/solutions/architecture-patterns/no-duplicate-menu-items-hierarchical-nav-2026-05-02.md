...
severity: high
applies_when:
  - Adding a new section to the Admin or Intelligence sidebar
  - Brainstorming where a feature lives in the navigation tree
  - A feature could plausibly belong in more than one nav section
tags: navigation, ux, admin-sidebar, intelligence-sidebar, information-architecture
---
...
During IA brainstorming for the H+ Admin sidebar (Sources, Resources) and Intelligence
(Knowledge Registry), a "Knowledge Base" item was proposed in two places simultaneously:

1. `Intelligence → Rebecca AI Assistant → Knowledge Base` — already existed
2. `Intelligence → Knowledge Registry → Knowledge Base` — newly proposed
...
|---|---|
| Platform documentation the AI assistant reads | `Intelligence → Rebecca AI Assistant → Knowledge Base` |
| External data / research the app reads from | `Admin → Sources → [sub-item]` |
| Analyst-generated calibration output | `Intelligence → [Analyst section or dedicated item]` |
...
- Every time a new sidebar item is proposed for `AdminSidebar.tsx` or `IntelligenceSidebar.tsx`
- Every time a brainstorm doc or skill proposes a nav tree — check for duplicates before
  finalising
- When reviewing PRs that touch sidebar section unions (`AdminSection`, `IntelligenceSection`)
...
"Knowledge Registry" was removed from the Intelligence tree entirely. It had no
unique content:
...
- Assumption Guidance → Analyst output, lives in Intelligence as its own item
