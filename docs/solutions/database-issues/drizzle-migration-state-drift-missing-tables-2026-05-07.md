...
This repo uses a two-layer design for DB migrations:

| Layer | Location | When it runs |
|-------|----------|--------------|
| **Drizzle SQL migrations** | `artifacts/api-server/migrations/*.sql` + `meta/_journal.json` | Once at boot via `migrate()` from `drizzle-orm/node-postgres/migrator` |
