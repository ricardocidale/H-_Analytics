# H+ Analytics — Session Notes Archive: April 2026

Archived from `memory.md` on 2026-05-10.
Contains all `### ... (April 2026) — COMPLETED` sections moved out of the active memory file.

---

### CompanyAssumptions → 7 Horizontal Tabs (April 2026) — COMPLETED
- **Change**: Refactored `client/src/pages/CompanyAssumptions.tsx` from a long single-scroll page into 7 horizontal tabs using shadcn `Tabs`. Active tab synced to URL via `?tab=` query param (replaceState, not pushState — back button exits page rather than cycling tabs).
- **Tabs**: 1) Setup, 2) Funding, 3) Revenue Model, 4) Compensation, 5) Overhead, 6) Tax & Exit, 7) Property Defaults.
- **State model**: Single shared `formData` + `handleSave` + all hooks remain at page level. Tabs are pure visual organization. Do NOT split form state per tab.
- **Pinned outside tabs**: PageHeader, IntelligenceStatusBar, FirstVisitBanner, ResearchTheater overlay, SummaryFooter (always-on totals), bottom SaveButton.
- **Sticky tab bar**: `sticky top-0 z-10 bg-background/85 backdrop-blur` so tabs stay reachable on long sub-pages (Revenue Model alone is 438 lines).
- **ScrollReveal removed inside tabs** — was causing re-animation flicker on tab switches. AnimatedPage kept at outer level.
- **Field moves**:
  - `depreciationYears` moved into `CompanySetupSection.tsx` as a Card next to the Inflation Card (still wrapped in `GovernedFieldWrapper` with `GOVERNED_FIELDS.depreciationYears` metadata).
  - `daysPerMonth` UI **deleted** from this page — already exists in Admin → App Defaults → Market & Macro as the single source of truth (`MarketMacroTab.tsx`). The schema field, API, and calc engine usage are unchanged.
- **Test ids preserved**: every existing input/slider `data-testid` retained. New ids added: `tabs-company-assumptions`, `tab-{name}`, `tab-content-{name}`.
- **Tab → component mapping**: see updated docstring in `client/src/components/company-assumptions/index.ts`.

### Refinance Appraisal: Cost-Basis (April 2026) — COMPLETED
- **Change**: Refinancing now appraises property at **cost basis** (purchasePrice + buildingImprovements) instead of income-approach (stabilizedNOI / exitCapRate).
- **Files changed**: `engine/property/refinance-pass.ts` (server path — uses `{ method: "direct", property_value_at_refi: costBasisValue }`), `engine/debt/loanCalculations.ts` (client path — `costBasisValue * refiLTV`).
- **Impact**: Refi proceeds dropped dramatically (e.g., Jano Grande: $6.9M → $1.2M). Portfolio IRR dropped from ~50% to ~41%.
- **Exit values still use NOI/cap rate**: Only the refi appraisal changed. Exit/sale valuation at disposition still uses `annualizedNOI / exitCapRate`.

### Photo Pipeline Changes (April 2026) — COMPLETED
- **Carousel removed**: Property cards now show a single hero image (chosen by user/admin), not a carousel. Removed Carousel/CarouselApi imports and auto-cycling logic from `PortfolioPropertyCard.tsx`.
- **Auto-enhance on upload**: Every photo uploaded via `POST /api/properties/:id/photos` is automatically sent through the AI enhancement pipeline (Replicate photo-upscale) in the background. The enhanced version becomes the default used everywhere. Pass `skipEnhancement: true` in request body to opt out.
- **Enhanced image priority**: `HeroImage` and property cards prefer `enhancedImageData` over raw upload when available.
- **Medellín Duplex address updated**: Sur-185 #16a, El Poblado (neighborhood), Medellín, Antioquia, Colombia
- **Property detail maps collapsible**: Maps/satellite views on `PropertyDetail.tsx` are now behind a collapsible chevron ("Location & Maps") — collapsed by default. Financial reporting is the primary focus of this page.
- **Carousel kept in codebase**: `carousel.tsx` component remains available for future use (photo album, etc.) — only removed from property cards.
...
### Audit Remediation Sprint (April 10, 2026) — COMPLETED
- **Scope**: Remediated all high and medium findings from audits #318–#328
- **H-001 (#328)**: Flaky PDF test timeout — increased to 15s in `pdf-render.snapshot.test.ts:364`
- **H-001 (#325)**: FormulaDetailRow duplicated 3× — extracted shared `FormulaDetailStringRow` to `financial-table/specialized-rows.tsx`, removed local copies from `YearlyCashFlowStatement.tsx` and `YearlyIncomeStatement.tsx`
- **M-005 (#326)**: RichBlockRenderers hardcoded hex — added CSS custom properties `--brand-navy`/`--brand-teal`/`--brand-gold` to `:root` and `.dark`, registered in `@theme inline` block, replaced all 15+ hardcoded `#112548`/`#0091AE`/`#FDB817` with Tailwind `brand-navy`/`brand-teal`/`brand-gold` utility classes
- **M-001 (#327/#326)**: Non-compliant catch blocks — bulk-fixed `: unknown` annotation across 134 files (client + server), zero violations remaining
- **M-003 (#328)**: Stale `NO_EXCEL_GUARANTEE.md` — updated test count from 384 → 4,463
- **L-002 (#327)**: `lib/api/index.ts:26` catch(e) → catch(e: unknown)
- Health: ALL CLEAR — 4,463 tests (183 files), TypeScript 0 errors, Lint PASS 0 errors, Verification UNQUALIFIED

### Type Safety & Guardrail Polish (April 10, 2026) — COMPLETED
- **T001**: Replaced 8 `any` types across `property-detail/types.ts` (6) and `YearlyIncomeStatement.tsx` (2) with proper `Property`, `GlobalResponse`, and `MonthlyFinancials` types
- **T002**: Added 3 new Quick Audit scanners: #11 catch compliance (`: unknown`), #12 brand hex audit, #13 prop `: any` tracker
- **T003**: Updated error-handling skill with `catch (error: unknown)` mandate, code examples, and checklist item
- Quick Audit now has 13 checks (was 10)
- Health: ALL CLEAR — 4,463 tests (183 files), TypeScript 0 errors, Lint PASS 0 errors
...
### T013-T015 pgvector Financial Intelligence (April 2026) — COMPLETED + ULTRAPLAN REVISION
...
- **Findings (all verified safe)**:
  ...
  - Knowledge Base: CRUD with pgvector sync, version history + rollback all working correctly
...
- **Fixes applied**:
  ...
  - Fixed empty catch block in rebecca.ts KB stats pgvector vector count — now logs warning
...
- **T002 — pgvector-service.ts split** (833→354 lines): Extracted 12 domain indexing functions to `server/ai/pgvector-indexing.ts` (466 lines). Main file re-exports for backward compatibility.
...
- **T005 — intelligence.ts split** (1131→224 lines): Extracted `intelligence-sources.ts`, `intelligence-scheduled.ts`, `intelligence-pgvector.ts`, `intelligence-qa.ts` (QA preview/live-test + key rotation routes).
- **New files**: `server/routes/chat-prompts.ts`, `server/routes/chat-insight.ts`, `server/routes/admin/intelligence-sources.ts`, `server/routes/admin/intelligence-scheduled.ts`, `server/routes/admin/intelligence-pgvector.ts`, `server/routes/admin/intelligence-qa.ts`, `server/storage/financial-sharing.ts`, `server/storage/financial-fees.ts`
- Health check: ALL CLEAR — 0 TS errors, 4,054 tests (173 files), verification UNQUALIFIED
...
- **Rebecca chatbot skill updated**: `.claude/skills/rebecca-chatbot/SKILL.md` — comprehensive rewrite covering all Tasks #305-#307 (personality, guardrails, KB CRUD, rich blocks, 5 admin tabs, pgvector sync, constraints table)
...
- **pgvector sync**: Non-blocking `syncKBEntryTopgvector()` helper upserts to `knowledge-base` namespace with ID pattern `admin-kb:{entryId}`. Active entries upserted, inactive entries deleted from vectors. Delete route removes vectors.
...
- **Code review fix**: Inactive KB entries now deleted from pgvector (not upserted) on toggle/rollback — prevents deactivated content from being retrieved by Rebecca.
...
- **SystemIntelligenceStatus wired into EngineDashboard**: Imported and rendered below CoverageAnalyticsDashboard. Shows LLM vendor availability, pgvector namespace stats, knowledge base health, missing API key detection, namespace re-index/clear actions.
...
- **Business-model-tagged pgvector guidance**: `indexAssumptionGuidance` and `retrieveSimilarGuidance` now accept `businessModel` param. Metadata includes `businessModel` tag. Query text includes business model for better embedding similarity. Returns include `businessModel` field
...
- **Static map endpoint**: `GET /api/geospatial/static-map?lat=&lng=&zoom=&w=&h=` — proxies Google Maps Static API when `GOOGLE_MAPS_API_KEY` present; otherwise composites ArcGIS World Imagery tiles via Sharp. Two-step Sharp pipeline required: first `composite(tiles).png().toBuffer()`, then `sharp(buffer).extract().composite([marker]).jpeg()`. Single-chain `create().composite().extract()` silently drops tiles due to Sharp's deferred execution. Caches 24h, caps dimensions at 640px. Requires `requireAuth`.
- Health check: ALL CLEAR — 0 TS errors, 3980 tests pass, verification UNQUALIFIED
...
- **Storage methods**: listEngineSuggestedLines (with status filter), getEngineSuggestedLineById, createEngineSuggestedLine, approveEngineSuggestedLine (+ pgvector indexing), rejectEngineSuggestedLine, countEngineSuggestedLines
...
- **Auto-refresh toggle switch**: PropertyEdit + CompanyAssumptions have an "Auto" switch next to the regenerate/run-research button
  - Shared hook: `client/src/hooks/use-auto-refresh-intelligence.ts`
  - Persists preference to `localStorage` under `hp-auto-refresh-intelligence` key (shared across all entities)
  - When ON: auto-triggers `generateResearch()` on page entry if status is missing/stale/very_stale (one-shot per visit via `useRef` guard)
  - When OFF: shows the intelligence prompt dialog (PropertyEdit only) instead of auto-firing
  - Guards: `entityReady && !isDirty && !isGenerating`
  - Tooltip explains behavior on hover

### Logo Management Redesign (April 2026) — COMPLETED
- **Two-card layout**: Admin LogosTab split into "Management Company Logos" (2-col span) + "App Identity" (1-col span)
- **`isDefault` flag**: Marks the management company default logo (logo pool). Set via star icon on logo cards.
- **`isAppLogo` flag**: Marks the platform identity logo (sidebar/login/headers). Set via App Identity card's logo picker grid.
- **Both flags are independent**: Same logo can have both, or different logos can hold each.
- **Delete protection**: Can't delete a logo if it has `isDefault` or `isAppLogo` flag.
- **Backend endpoints**: `PATCH /api/logos/:id/default` (set mgmt default), `GET /api/app-branding` (read app logo), `PATCH /api/app-branding` (set app logo)
- **Migration**: `app-logo-001.ts` adds `isAppLogo` boolean column to `logos` table
- **Seed data**: Logos renamed to generic hospitality names. Companies trimmed to Norfolk AI + General only.
- **AI image generation**: `graphicsLlm` domain default changed to `openai/gpt-image-1`. Prompt enhancement uses `aiUtilityLlm` (Gemini text model) to avoid model/vendor mismatch.
- **Files**: `LogosTab.tsx`, `admin/hooks.ts`, `admin/types.ts`, `server/storage/admin.ts`, `server/routes/branding.ts`, `server/migrations/app-logo-001.ts`
...
- **6 stages**: Backend/Security, Database/Schema, Financial Engine, Frontend/React, AI/pgvector, Docs/UI
- **Overall**: STRONG with targeted improvements needed
- **P0 FIXED**: (1) `cleanupPropertyVectors()` in pgvector-service.ts — property deletion now cleans 5 namespaces, (2) `Promise.race` with `AI_GENERATION_TIMEOUT_MS` in research orchestrator
...
- **Scope**: 35 TypeScript files (~4,700 lines) across shared/ (17 schema files, 7 constants files, 9 utility/type files, 2 legacy files)
...
- **Scope**: 65 files (~13,356 lines) across server/ai/ (21), server/services/ (20), server/integrations/ (5), server/notifications/ (2), server/document-ai/ (2), server/image/ (2), server/seeds/ (9), server/scripts/ (4)
- **Verdict**: PASS — 0 critical, 1 high, 4 medium, 6 low findings
- **Key findings**: (H) Notification actionUrl always undefined (dead ternary); (M) Duplicate BaseIntegrationService classes with divergent resilience; (M) 6 `as any` in research-scheduler JSONB payloads; (M) Hardcoded 8s sleep on pgvector index creation; (M) 5 `as any` in rebecca-context-builder for missing type fields; (L) 8 empty catch blocks; (L) Fragile isTransientError string matching; (L) Unbounded geospatial caches; (L) Document AI fallback returns hardcoded fake data; (L) Resend creates new client per email; (L) Synthesis hardcodes Anthropic despite configurable model
- **Positives**: N+1 parallel research synthesis (dual analysts + API validation + Opus synthesis), Promise.allSettled fault isolation in 14-source aggregator, data recency validation (90/365 day thresholds), proper circuit breakers, mutex-based pgvector index creation, branded responsive email templates, RapidAPI 3-slot key routing
- **as any count**: 14 in scope (research-scheduler 6, rebecca-context-builder 5, research-client 2, pgvector-service 1)
...
### Opus Audit #324: Client Pages, Routing & Core Hooks (April 2026) — COMPLETED
- **Report**: `docs/audits/324-client-pages-routing-core-hooks-findings.md`
- **Scope**: ~119 files (~19,735 lines) across App.tsx (668L), pages/ (33+59 sub-files, 15,008L), hooks/ (8 files, 915L), features/ (17 files, 3,122L), types/ (1 file, 22L)
- **Verdict**: PASS — 0 critical, 0 high, 5 medium, 6 low findings
- **Key findings**: (M) PropertyEdit.tsx uses `useState<any>` for form draft; (M) useServerFinancials useMemo dependency uses `.length` instead of stable ref; (M) No dynamic document.title on page navigation; (M) No 404 handling for invalid entity IDs; (M) CompanyIcpDefinition has 12 `as any` from untyped GlobalResponse ICP fields; (L) App.tsx any[] state/ref; (L) useUpload options in useCallback dep; (L) stale closure in handleResearchComplete/IdleAutoSave; (L) 28 `as any` across pages (21 avoidable via GlobalResponse + research content typing); (L) `prop as unknown as LoanParams` double cast
- **Positives**: 100% lazy loading (all 24 pages + Login Three.js deferred), triple-layer error boundaries (Sentry → ErrorBoundary → FinancialErrorBoundary), 4-tier unsaved-work protection (beforeunload + navigation guard + logout dialog + idle auto-save), server-side financial computation, 12 legacy redirects preserved, 97% data-testid coverage, hooks are clean and focused, feature modules well-structured
...
- **Scope**: 137 files (~18,922 lines) across 14 directories — dashboard/ (23), statements/ (4), financial-table/ (7), charts/ (5), portfolio/ (3), investment/ (3), financing/ (5), sensitivity/ (6), scenarios/ (6), funding/ (4), lib/financial/ (16), lib/audits/ (13), lib/charts/ (14), plus key hooks
- **Verdict**: PASS — 0 critical, 1 high, 4 medium, 5 low findings
...
- **as any count**: 5 in scope (usePortfolioFinancials 2 + jsPDF 3 — all justified)
...
- **Positives**: Exceptional admin architecture (106 files, 5-group sidebar, 35+ typed sections, 14 SECTION_REDIRECTS); admin testid coverage 793 in 89 files (8.9/file); ICP config system fully declarative with zero as-any; research streams use AbortController + catch(error:unknown); Lucide hardcoded as single icon set (Phosphor/Material removed); verification UI 11 files; property edit form sections well-documented with GAAP references
