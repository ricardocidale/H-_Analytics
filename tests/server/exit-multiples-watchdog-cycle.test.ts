/**
 * Tests for the scheduled Exit-Multiples Watchdog cycle.
 *
 * Mirror of `capital-raise-watchdog-cycle.test.ts`. Mocks the LLM
 * synthesizer (`researchExitMultiples`), the storage audit-log reader,
 * and the snapshot applier. Verifies:
...
vi.mock("../../server/ai/analyst-table-refresh", async () => {
  ...
  return {
    ...
    applyWatchdogExitMultiplesSnapshot: vi.fn(),
  };
});
...
import { runExitMultiplesWatchdogCycle } from "../../server/ai/ambient/exit-multiples-watchdog";

const mockedStorage = storage as unknown as {
  getExitMultiples: ReturnType<typeof vi.fn>;
  getRecentAnalystRefreshAuditLogs: ReturnType<typeof vi.fn>;
};
const mockedResearch = researchExitMultiples as unknown as ReturnType<typeof vi.fn>;
const mockedApply = applyWatchdogExitMultiplesSnapshot as unknown as ReturnType<typeof vi.fn>;
...
const SAMPLE_EVIDENCE = [
  { source: "SaaS Capital Index 2024", url: "https://saascapital.com/x", finding: "Median public SaaS multiple 6.2x ARR" },
  { source: "PitchBook M&A 2024",      url: "https://pitchbook.com/y",   finding: "E-commerce median 1.9x revenue" },
  { source: "CB Insights Verticals",   url: "https://cbinsights.com/z",  finding: "Marketplace GMV-take median 5x" },
];

beforeEach(() => {
  vi.clearAllMocks();
  delete process.env.EXIT_MULTIPLES_WATCHDOG_CADENCE_HOURS;
  ...
  mockedApply.mockResolvedValue({
    ...
    auditId: 42,
    appliedDimensions: ["saas", "ecommerce", "marketplace"],
    skippedDimensions: [],
    recordedAt: new Date(),
  });
});

describe("runExitMultiplesWatchdogCycle", () => {
  ...
  it("ignores cadence when force=true", async () => {
    ...
    const outcome = await runExitMultiplesWatchdogCycle({ force: true });
  ...
  it("widens the audit lookup window when a very long cadence is configured", async () => {
    process.env.EXIT_MULTIPLES_WATCHDOG_CADENCE_HOURS = String(24 * 60); // 60 days
    mockedResearch.mockResolvedValue({
      proposedRanges: SAMPLE_RANGES,
      narration: ["…"],
      sourceCount: 3,
    ...
    await runExitMultiplesWatchdogCycle();
  ...
  it("drops dimensions whose three values are all null before applying", async () => {
    mockedResearch.mockResolvedValue({
      proposedRanges: [
        ...SAMPLE_RANGES,
        { dimensionKey: "phantom", label: "Phantom", unit: "x_revenue",
  ...
  it("respects EXIT_MULTIPLES_WATCHDOG_CADENCE_HOURS override", async () => {
    process.env.EXIT_MULTIPLES_WATCHDOG_CADENCE_HOURS = "1"; // 1h cadence
    mockedStorage.getRecentAnalystRefreshAuditLogs.mockResolvedValue([
      {
        id: 11,
        tableId: "exit_multiples",
