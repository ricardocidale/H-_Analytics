/**
 * Post-synthesis quality validator for the Compensation Specialist (G3 IB
 * req #9).
 *
 ...
 * parse time.
 *
 * Note on collapsed ranges: compensation dimensions emit USD or count
 * integers. The `low === high` sentinel still flags genuine point estimates
 * because no honest Opus emission for a $500K+ wide partner-comp band or a
 * 2-FTE-wide staffing band collapses to a single value. Funding's logic
...
export function validateSynthesisOutput(
  output: CompensationSpecialistOutput,
  ...
  const missingKeys = COMPENSATION_DIMENSION_KEYS.filter((k) => !emittedKeys.has(k));
  if (missingKeys.length > 0) {
    return {
      pass: false,
      regressReason: `Synthesis omitted dimensions: ${missingKeys.join(", ")}. Emit exactly these 5 keys: ${COMPENSATION_DIMENSION_KEYS.join(", ")}.`,
    };
  }
