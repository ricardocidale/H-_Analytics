/**
 * Post-synthesis quality validator for the Property-Defaults Specialist (Phase
 * 2 IB req #9).
 *
 ...
 * parse time.
 *
 * Note on collapsed ranges: all 4 dimensions emit fractions (0.65 = 65%).
 * The `low === high` sentinel flags genuine point estimates because no honest
 * Opus emission for an expense/rate band collapses to a single value — the
 * bands are wide enough across the property comp set that a degenerate range
 * signals model-output failure, not market consensus.
 */

import type { PropertyDefaultsSpecialistOutput } from "./mgmt-co-property-defaults-output-schema";
...
export function validateSynthesisOutput(
  output: PropertyDefaultsSpecialistOutput,
  ...
  const missingKeys = PROPERTY_DEFAULTS_DIMENSION_KEYS.filter((k) => !emittedKeys.has(k));
  if (missingKeys.length > 0) {
    return {
      pass: false,
      regressReason: `Synthesis omitted dimensions: ${missingKeys.join(", ")}. Emit exactly these 4 keys: ${PROPERTY_DEFAULTS_DIMENSION_KEYS.join(", ")}.`,
    };
  }
